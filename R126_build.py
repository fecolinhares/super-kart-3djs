#!/usr/bin/env python3
"""Build Super Kart Hero V2 R126 from a class-level authored cockpit revision.
...
def parse_args():
    ...
    p.add_argument("--revision", default="R126")
...
def create_cockpit_driver_r126(cfg):
    """R126: connected stylized pilot shell with explicit control-chain attachments."""
    ...
    loft("R126_CockpitWell",[(.48,0,.42,.30,.16),(.22,0,.405,.32,.18),(-.10,0,.40,.28,.16),(-.36,0,.41,.21,.11)],max(10,cfg["body_sides"]-4),"rubber_dark",2.4)
    loft("R126_PilotShell",[(.19,0,.54,.235,.14),(.10,0,.67,.255,.19),(.04,0,.82,.31,.235),(.10,0,.94,.255,.15)],max(16,cfg["body_sides"]//1),"paint_secondary",2.0)
    # Sculpted armor collar/shoulders connect directly into the shell's upper section.
    for side in (-1,1):
        ellipsoid("R126_ShoulderArmor",(side*.205,.075,.915),(.115,.135,.105),"paint_primary",cfg["sphere_seg"],cfg["sphere_ring"])
        tube_path("R126_Harness",[(side*.16,-.005,.98),(side*.055,-.045,.79),(side*.13,.015,.58)],.020,"metal_warm",cfg["tube_sides"],.012)
        tube_path("R126_SideArmor",[(side*.22,.10,.88),(side*.27,.02,.73),(side*.20,-.08,.61)],.032,"paint_primary",cfg["tube_sides"],.020)
    # Neck and helmet are a tapered wearable shell, not a floating sphere.
    loft("R126_NeckShell",[(-.01,0,1.00,.095,.08),(.04,0,1.08,.11,.105)],max(12,cfg["body_sides"]//2),"paint_primary",2.0)
    loft("R126_HelmetShell",[(-.12,0,1.10,.12,.105),(-.04,0,1.20,.18,.17),(.07,0,1.29,.14,.10)],max(16,cfg["body_sides"]//1),"paint_primary",2.0)
    loft("R126_HelmetVisor",[(-.145,0,1.145,.085,.04),(-.125,0,1.205,.13,.055),(-.08,0,1.255,.095,.03)],max(12,cfg["body_sides"]//2),"rubber_dark",2.0)
    for side in (-1,1):
        cylinder_x("R126_VisorPivot",(side*.135,-.13,1.19),.017,.025,"metal_warm",cfg["rim_seg"])
    tube_path("R126_HelmetFrame",[(-.135,-.13,1.14),(-.11,-.16,1.22),(0,-.17,1.27),(.11,-.16,1.22),(.135,-.13,1.14)],.014,"metal_warm",cfg["tube_sides"],.010)
    tube_path("R126_ChinGuard",[(-.12,-.12,1.10),(0,-.17,1.065),(.12,-.12,1.10)],.030,"paint_primary",cfg["tube_sides"],.020)
    tube_path("R126_Collar",[(-.17,.01,1.03),(0,.07,1.00),(.17,.01,1.03)],.030,"metal_warm",cfg["tube_sides"],.020)
    # Legs and boots visibly terminate in the pedal shelf.
    triangulated_box("R126_PedalShelf",(0,-.46,.29),(.44,.29,.035),"metal_warm")
    for side in (-1,1):
        tube_path("R126_Thigh",[(side*.14,.10,.58),(side*.17,-.15,.46),(side*.13,-.36,.35)],.078,"paint_secondary",cfg["tube_sides"],.060)
        tube_path("R126_Shin",[(side*.13,-.34,.36),(side*.11,-.49,.33),(side*.095,-.56,.335)],.054,"paint_primary",cfg["tube_sides"],.040)
        ellipsoid("R126_Boot",(side*.095,-.565,.35),(.068,.10,.052),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    ...
    tube_path("R126_SteeringRing",wheel_pts,.022,"paint_primary",cfg["tube_sides"],closed=True)
    cylinder_x("R126_SteeringHub",(0,-.46,.82),.040,.060,"metal_warm",cfg["rim_seg"])
    cylinder_x("R126_SteeringCap",(0,-.46,.82),.016,.066,"paint_secondary",cfg["rim_seg"])
    tube_path("R126_SteeringColumn",[(0,-.46,.82),(0,-.42,.66),(0,-.42,.61)],.020,"accent_emissive",cfg["tube_sides"],.015)
    for side in (-1,1):
        tube_path("R126_UpperArm",[(side*.21,.10,.91),(side*.31,-.03,.84),(side*.23,-.17,.82)],.056,"paint_primary",cfg["tube_sides"],.044)
        tube_path("R126_Forearm",[(side*.23,-.17,.82),(side*.18,-.445,.82)],.047,"paint_secondary",cfg["tube_sides"],.038)
        a=math.radians(30 if side>0 else 150); gx=.17*math.cos(a); gz=.82+.17*math.sin(a)
        hand=ellipsoid("R126_Hand",(gx,-.455,gz),(.039,.030,.047),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"]); hand["grip_angle_deg"]=30 if side>0 else 150
    # R126 class change: replace the isolated arc/rollbar reading with a shallow,
    ...
    # R126: remove the dominant opaque cowl slab; retain only the compact authored lens below.
    ...
    for side in (-1,1):
        tube_path("R126_CowlShoulderRail",[(side*.33,-.79,.655),(side*.35,-.70,.70),(side*.27,-.53,.79)],.018,"paint_secondary",cfg["tube_sides"],.012)
        fast=ellipsoid("R126_CowlMount",(side*.335,-.76,.66),(.018,.014,.012),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"])
        fast["mount_type"]="integrated_cowl_shell_mount"


def create_cockpit_driver(cfg):
    if cfg["level"] == 0:
        create_cockpit_driver_r126(cfg)
        return
    ...
    if cfg["level"] == 0:
        ...
        # R126: no freestanding A-pillar rods; compact lens is carried by the cowl rail.
        tube_path("KartWindshieldCowlRail",[(-.18,-.58,.62),(-.09,-.61,.615),(0,-.64,.61),(.09,-.61,.615),(.18,-.58,.62)],.018,"paint_secondary",cfg["tube_sides"],.010)
        for side in (-1,1):
...
def main():
    ...
    pilot=bpy.data.objects.get('LOD0_R126_PilotShell')
    if pilot:
    ...
    out=os.path.abspath(args.output or os.path.join(os.path.dirname(__file__), "hero-kart-v2-R126.blend")); os.makedirs(os.path.dirname(out),exist_ok=True)
