#!/usr/bin/env python3
"""Build Super Kart Hero V2 R142 from a class-level authored cockpit revision.
...
def parse_args():
    ...
    p.add_argument("--revision", default="R142")
...
def create_cockpit_driver_r142(cfg):
    """R142: connected stylized pilot shell with explicit control-chain attachments."""
    ...
    loft("R142_CockpitWell",[(.48,0,.42,.30,.16),(.22,0,.405,.32,.18),(-.10,0,.40,.28,.16),(-.36,0,.41,.21,.11)],max(10,cfg["body_sides"]-4),"rubber_dark",2.4)
    # R142: pilot shell is widened at the hips and narrowed through the waist,
    # with a dedicated shoulder bridge to read as a seated racer rather than a capsule.
    loft("R142_PilotShell",[(.19,0,.54,.235,.14),(.10,0,.66,.285,.18),(.04,0,.80,.235,.22),(.10,0,.93,.31,.15)],max(16,cfg["body_sides"]//1),"paint_secondary",2.0)
    loft("R142_ChestShell",[(.02,0,.78,.20,.14),(.08,0,.88,.26,.17),(.11,0,.96,.20,.10)],max(14,cfg["body_sides"]//2),"paint_primary",2.0)
    # Sculpted armor collar/shoulders connect directly into the shell's upper section.
    for side in (-1,1):
        ellipsoid("R142_ShoulderArmor",(side*.205,.075,.915),(.115,.135,.105),"paint_primary",cfg["sphere_seg"],cfg["sphere_ring"])
        tube_path("R142_Harness",[(side*.16,-.005,.98),(side*.055,-.045,.79),(side*.13,.015,.58)],.020,"metal_warm",cfg["tube_sides"],.012)
        tube_path("R142_SideArmor",[(side*.22,.10,.88),(side*.27,.02,.73),(side*.20,-.08,.61)],.032,"paint_primary",cfg["tube_sides"],.020)
    # Neck and helmet are a tapered wearable shell, not a floating sphere.
    loft("R142_NeckShell",[(-.01,0,.96,.10,.09),(.04,0,1.10,.115,.11)],max(12,cfg["body_sides"]//2),"paint_primary",2.0)
    loft("R142_HelmetShell",[(-.12,0,1.07,.12,.105),(-.04,0,1.17,.18,.17),(.07,0,1.26,.14,.10)],max(16,cfg["body_sides"]//1),"paint_primary",2.0)
    loft("R142_HelmetVisor",[(-.145,0,1.145,.085,.04),(-.125,0,1.205,.13,.055),(-.08,0,1.255,.095,.03)],max(12,cfg["body_sides"]//2),"rubber_dark",2.0)
    for side in (-1,1):
        cylinder_x("R142_VisorPivot",(side*.135,-.13,1.19),.017,.025,"metal_warm",cfg["rim_seg"])
    tube_path("R142_HelmetFrame",[(-.135,-.13,1.14),(-.11,-.16,1.22),(0,-.17,1.27),(.11,-.16,1.22),(.135,-.13,1.14)],.014,"metal_warm",cfg["tube_sides"],.010)
    tube_path("R142_ChinGuard",[(-.12,-.12,1.10),(0,-.17,1.065),(.12,-.12,1.10)],.030,"paint_primary",cfg["tube_sides"],.020)
    tube_path("R142_Collar",[(-.17,.01,1.03),(0,.07,1.00),(.17,.01,1.03)],.030,"metal_warm",cfg["tube_sides"],.020)
    # Legs and boots visibly terminate in the pedal shelf.
    triangulated_box("R142_PedalShelf",(0,-.46,.29),(.44,.29,.035),"metal_warm")
    for side in (-1,1):
        tube_path("R142_Thigh",[(side*.14,.10,.58),(side*.17,-.15,.46),(side*.13,-.36,.35)],.078,"paint_secondary",cfg["tube_sides"],.060)
        tube_path("R142_Shin",[(side*.13,-.34,.36),(side*.11,-.49,.33),(side*.095,-.56,.335)],.054,"paint_primary",cfg["tube_sides"],.040)
        ellipsoid("R142_Boot",(side*.095,-.565,.35),(.068,.10,.052),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    ...
    tube_path("R142_SteeringRing",wheel_pts,.022,"paint_primary",cfg["tube_sides"],closed=True)
    cylinder_x("R142_SteeringHub",(0,-.46,.82),.040,.060,"metal_warm",cfg["rim_seg"])
    cylinder_x("R142_SteeringCap",(0,-.46,.82),.016,.066,"paint_secondary",cfg["rim_seg"])
    tube_path("R142_SteeringColumn",[(0,-.46,.82),(0,-.42,.66),(0,-.42,.61)],.020,"accent_emissive",cfg["tube_sides"],.015)
    for side in (-1,1):
        tube_path("R142_UpperArm",[(side*.21,.10,.91),(side*.31,-.03,.84),(side*.23,-.17,.82)],.056,"paint_primary",cfg["tube_sides"],.044)
        tube_path("R142_Forearm",[(side*.23,-.17,.82),(side*.18,-.445,.82)],.047,"paint_secondary",cfg["tube_sides"],.038)
        a=math.radians(30 if side>0 else 150); gx=.17*math.cos(a); gz=.82+.17*math.sin(a)
        hand=ellipsoid("R142_Hand",(gx,-.455,gz),(.039,.030,.047),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"]); hand["grip_angle_deg"]=30 if side>0 else 150
    # R142 class change: replace the isolated arc/rollbar reading with a shallow,
    ...
    # R142: remove the dominant opaque cowl slab; retain only the compact authored lens below.
    ...
    # R142: the review windshield is now a single flush lens with no exposed rail rods.
    # Cowl connection is represented by the lens edge and the two flush mount points only.
    for side in (-1,1):
        fast=ellipsoid("R142_CowlMount",(side*.17,-.59,.64),(.018,.010,.010),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"])
        fast["mount_type"]="integrated_cowl_shell_mount"


def create_cockpit_driver(cfg):
    if cfg["level"] == 0:
        create_cockpit_driver_r142(cfg)
        return
    ...
    ellipsoid("DriverHeadInternal",(0,.02,1.16),(.078,.060,.096),"paint_secondary",cfg["sphere_seg"],cfg["sphere_ring"])
    loft("HelmetShell",[(-.08,0,1.10,.10,.10),(.00,0,1.20,.145,.16),(.10,0,1.28,.105,.085)],max(12,cfg["body_sides"]//2),"paint_primary",2.0)
    loft("HelmetVisor",[(-.125,0,1.135,.075,.040),(-.115,0,1.20,.105,.050),(-.08,0,1.245,.075,.030)],max(10,cfg["body_sides"]//3),"rubber_dark",2.0)
    for side in (-1,1):
    ...
    if cfg["level"] == 0:
        ...
        xs=[-.18,-.09,0,.09,.18]
        # R142: compact flush deflector, lowered into the cowl instead of a floating panel.
        rows=((.56,-.59),(.62,-.62),(.69,-.58),(.73,-.51))
        verts=[]
        for x in xs:
            bend=1.0-(abs(x)/.18)**2
        ...
        for i in range(len(xs)-1):
            a=i*3; b=(i+1)*3
            for r in range(3): faces.append((a+r,b+r,b+r+1,a+r+1))
        ...
        # R142: lens is flush-mounted directly to the cowl; no exposed rail.
        for side in (-1,1):
...
def main():
    ...
    pilot=bpy.data.objects.get('LOD0_R142_PilotShell')
    if pilot:
    ...
    # R142: re-anchor wearable helmet stack to the seated pilot shell for profile proof.
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R142_") and any(k in obj.name for k in ("Helmet", "Visor", "ChinBar", "Collar")):
            obj.location.z -= 0.10
    # R142: connected head-to-shoulder placement for the profile/clearance gate.
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R142_") and any(k in obj.name for k in ("Helmet", "Visor", "ChinBar", "Collar", "Neck")):
            obj.location.z -= 0.12
    # R142: move wearable head geometry in mesh space so profile proves contact with torso.
    shift=Matrix.Translation((0,0,-0.16))
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R142_") and any(k in obj.name for k in ("Helmet", "Visor", "ChinBar", "Collar", "Neck")) and obj.type == "MESH":
            obj.data.transform(shift)
    # R142: remove only confirmed floating helmet accessories; retain shell, visor and collar.
    for obj in list(bpy.data.objects):
        if obj.name.startswith("LOD0_R142_") and any(k in obj.name for k in ("HelmetFrame", "VisorPivot")):
            bpy.data.objects.remove(obj, do_unlink=True)
    # R142: remove only confirmed detached visor-frame/pivot objects by semantic component name.
    for obj in list(bpy.data.objects):
        if obj.type == "MESH" and any(k in obj.name for k in ("HelmetVisorFrame", "HelmetVisorPivot", "VisorFrame", "VisorPivot")):
            bpy.data.objects.remove(obj, do_unlink=True)
    # R142: remove detached visor geometry identified by component semantics; preserve shell/chin/collar.
    for obj in list(bpy.data.objects):
        if obj.type == "MESH" and any(k in obj.name for k in ("HelmetVisor", "VisorFrame", "VisorPivot")):
            bpy.data.objects.remove(obj, do_unlink=True)
    # R142: semantic cleanup of every detached visor/helmet accessory, independent of revision prefix.
    for obj in list(bpy.data.objects):
        if obj.type == "MESH" and any(k.lower() in obj.name.lower() for k in ("helmetvisor", "visorframe", "visorpivot")):
            bpy.data.objects.remove(obj, do_unlink=True)
    # R142: remove confirmed detached helmet accessories; retain main shell and internal head.
    for obj in list(bpy.data.objects):
        if obj.type == "MESH" and any(k.lower() in obj.name.lower() for k in ("chinbar", "helmetframe", "collar")):
            bpy.data.objects.remove(obj, do_unlink=True)
    # R142: replace the lofted helmet shell with one closed centered wearable volume.
    for obj in list(bpy.data.objects):
        if obj.type == "MESH" and any(k.lower() in obj.name.lower() for k in ("helmetshell", "helmetvisor", "visorframe", "visorpivot")):
            bpy.data.objects.remove(obj, do_unlink=True)
    global MATS
    bpy.ops.mesh.primitive_uv_sphere_add(segments=32, ring_count=16, location=(0,0.04,1.16))
    helmet=bpy.context.object; helmet.name="LOD0_R142_HelmetShellClean"; helmet.scale=(.14,.12,.16); bpy.ops.object.transform_apply(location=False,rotation=False,scale=True)
    helmet.data.materials.append(MATS["paint_primary"]); helmet["authored_component"]="HelmetShellClean"
    out=os.path.abspath(args.output or os.path.join(os.path.dirname(__file__), "hero-kart-v2-R142.blend")); os.makedirs(os.path.dirname(out),exist_ok=True)
