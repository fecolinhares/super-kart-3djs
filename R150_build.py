#!/usr/bin/env python3
"""Build Super Kart Hero V2 R150 from a class-level authored cockpit revision.
...
def parse_args():
    ...
    p.add_argument("--revision", default="R150")
...
def create_cockpit_driver_r150(cfg):
    """R150: connected stylized pilot shell with explicit control-chain attachments."""
    ...
    loft("R150_CockpitWell",[(.48,0,.42,.30,.16),(.22,0,.405,.32,.18),(-.10,0,.40,.28,.16),(-.36,0,.41,.21,.11)],max(10,cfg["body_sides"]-4),"rubber_dark",2.4)
    # R150: pilot shell is widened at the hips and narrowed through the waist,
    # with a dedicated shoulder bridge to read as a seated racer rather than a capsule.
    loft("R150_PilotShell",[(.19,0,.54,.235,.14),(.10,0,.66,.285,.18),(.04,0,.80,.235,.22),(.10,0,.93,.31,.15)],max(16,cfg["body_sides"]//1),"paint_secondary",2.0)
    loft("R150_ChestShell",[(.02,0,.78,.20,.14),(.08,0,.88,.26,.17),(.11,0,.96,.20,.10)],max(14,cfg["body_sides"]//2),"paint_primary",2.0)
    # Sculpted armor collar/shoulders connect directly into the shell's upper section.
    for side in (-1,1):
        ellipsoid("R150_ShoulderArmor",(side*.205,.075,.915),(.115,.135,.105),"paint_primary",cfg["sphere_seg"],cfg["sphere_ring"])
        tube_path("R150_Harness",[(side*.16,-.005,.98),(side*.055,-.045,.79),(side*.13,.015,.58)],.020,"metal_warm",cfg["tube_sides"],.012)
        tube_path("R150_SideArmor",[(side*.22,.10,.88),(side*.27,.02,.73),(side*.20,-.08,.61)],.032,"paint_primary",cfg["tube_sides"],.020)
    # Neck and helmet are a tapered wearable shell, not a floating sphere.
    loft("R150_NeckShell",[(-.01,0,.96,.10,.09),(.04,0,1.10,.115,.11)],max(12,cfg["body_sides"]//2),"paint_primary",2.0)
    ellipsoid("R150_HelmetShell",(0,.035,1.18),(.15,.13,.17),"paint_primary",cfg["sphere_seg"],cfg["sphere_ring"])
    ellipsoid("R150_HelmetVisor",(0,-.010,1.205),(.082,.018,.045),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    # R150: shallow side-visible visor lens seated on the shell surface for profile proof.
    ellipsoid("R150_ProfileVisorPatch",(.145,-.005,1.205),(.012,.078,.050),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    for side in (-1,1):
        cylinder_x("R150_VisorPivot",(side*.135,.005,1.19),.017,.025,"metal_warm",cfg["rim_seg"])
    tube_path("R150_HelmetFrame",[(-.075,-.027,1.17),(-.082,-.029,1.205),(0,-.031,1.25),(.082,-.029,1.205),(.075,-.027,1.17)],.008,"metal_warm",cfg["tube_sides"],.006)
    tube_path("R150_ChinGuard",[(-.12,-.12,1.10),(0,-.17,1.065),(.12,-.12,1.10)],.030,"paint_primary",cfg["tube_sides"],.020)
    tube_path("R150_Collar",[(-.17,.01,1.03),(0,.07,1.00),(.17,.01,1.03)],.030,"metal_warm",cfg["tube_sides"],.020)
    # Legs and boots visibly terminate in the pedal shelf.
    triangulated_box("R150_PedalShelf",(0,-.46,.29),(.44,.29,.035),"metal_warm")
    for side in (-1,1):
        tube_path("R150_Thigh",[(side*.14,.10,.58),(side*.17,-.15,.46),(side*.13,-.36,.35)],.078,"paint_secondary",cfg["tube_sides"],.060)
        tube_path("R150_Shin",[(side*.13,-.34,.36),(side*.11,-.49,.33),(side*.095,-.56,.335)],.054,"paint_primary",cfg["tube_sides"],.040)
        ellipsoid("R150_Boot",(side*.095,-.565,.35),(.068,.10,.052),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    ...
    tube_path("R150_SteeringRing",wheel_pts,.022,"paint_primary",cfg["tube_sides"],closed=True)
    cylinder_x("R150_SteeringHub",(0,-.46,.82),.040,.060,"metal_warm",cfg["rim_seg"])
    cylinder_x("R150_SteeringCap",(0,-.46,.82),.016,.066,"paint_secondary",cfg["rim_seg"])
    tube_path("R150_SteeringColumn",[(0,-.46,.82),(0,-.42,.66),(0,-.42,.61)],.020,"accent_emissive",cfg["tube_sides"],.015)
    for side in (-1,1):
        tube_path("R150_UpperArm",[(side*.21,.10,.91),(side*.31,-.03,.84),(side*.23,-.17,.82)],.056,"paint_primary",cfg["tube_sides"],.044)
        tube_path("R150_Forearm",[(side*.23,-.17,.82),(side*.18,-.445,.82)],.047,"paint_secondary",cfg["tube_sides"],.038)
        a=math.radians(30 if side>0 else 150); gx=.17*math.cos(a); gz=.82+.17*math.sin(a)
        hand=ellipsoid("R150_Hand",(gx,-.455,gz),(.039,.030,.047),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"]); hand["grip_angle_deg"]=30 if side>0 else 150
    # R150 class change: replace the isolated arc/rollbar reading with a shallow,
    ...
    # R150: remove the dominant opaque cowl slab; retain only the compact authored lens below.
    ...
    # R150: the review windshield is now a single flush lens with no exposed rail rods.
    # Cowl connection is represented by the lens edge and the two flush mount points only.
    for side in (-1,1):
        fast=ellipsoid("R150_CowlMount",(side*.17,-.59,.64),(.018,.010,.010),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"])
        fast["mount_type"]="integrated_cowl_shell_mount"


def create_cockpit_driver(cfg):
    if cfg["level"] == 0:
        create_cockpit_driver_r150(cfg)
        return
    ...
    if cfg["level"] == 0:
        ...
        # R150: low, shallow cowl deflector; top edge remains below the helmet chin.
        ...
        # R150: lens is flush-mounted directly to the cowl; no exposed rail.
        for side in (-1,1):
...
def main():
    ...
    pilot=bpy.data.objects.get('LOD0_R150_PilotShell')
    if pilot:
    ...
    # R150: preserve the authored wearable helmet stack; do not collapse it into a head sphere.
    # Lower the complete helmet/visor/chin/collar assembly into the neck socket.
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R150_") and any(k in obj.name for k in ("Helmet", "Visor", "ChinBar", "Collar", "Neck")):
            obj.location.z -= 0.055
    # Make the visor visibly front-facing and continuous with the shell.
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R150_HelmetVisor"):
    ...
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R150_") and any(k in obj.name for k in ("NeckShell", "Collar")):
            obj["pilot_attachment"] = "neck_to_shoulder_bridge"
    out=os.path.abspath(args.output or os.path.join(os.path.dirname(__file__), "hero-kart-v2-R150.blend")); os.makedirs(os.path.dirname(out),exist_ok=True)
