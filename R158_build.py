#!/usr/bin/env python3
"""Build Super Kart Hero V2 R158 from a class-level authored cockpit revision.
...
def parse_args():
    ...
    p.add_argument("--revision", default="R158")
...
def create_cockpit_driver_r158(cfg):
    """R158: connected stylized pilot shell with explicit control-chain attachments."""
    ...
    loft("R158_CockpitWell",[(.48,0,.42,.30,.16),(.22,0,.405,.32,.18),(-.10,0,.40,.28,.16),(-.36,0,.41,.21,.11)],max(10,cfg["body_sides"]-4),"rubber_dark",2.4)
    # R158: pilot shell is widened at the hips and narrowed through the waist,
    # with a dedicated shoulder bridge to read as a seated racer rather than a capsule.
    loft("R158_PilotShell",[(.19,0,.54,.255,.15),(.10,0,.66,.305,.19),(.04,0,.80,.265,.23),(.10,0,.93,.32,.16)],max(16,cfg["body_sides"]//1),"paint_primary",2.0)
    # R158: one continuous shoulder yoke gives the suit a seated racer silhouette.
    tube_path("R158_SuitShoulderYoke",[(-.27,.075,.90),(-.18,.045,.96),(0,.025,.985),(.18,.045,.96),(.27,.075,.90)],.050,"paint_primary",cfg["tube_sides"],.038)
    loft("R158_ChestShell",[(.02,0,.73,.245,.17),(.08,0,.84,.31,.215),(.10,0,.93,.255,.145)],max(14,cfg["body_sides"]//2),"paint_primary",2.0)
    # R158: angular shoulder bridge joins the suit mass to the neck socket.
    tube_path("R158_IntegratedShoulderBridge",[(-.29,.06,.87),(-.20,.03,.95),(0,.015,.99),(.20,.03,.95),(.29,.06,.87)],.042,"paint_primary",cfg["tube_sides"],.032)
    # Sculpted armor collar/shoulders connect directly into the shell's upper section.
    for side in (-1,1):
        tube_path("R158_Harness",[(side*.16,-.005,.98),(side*.055,-.045,.79),(side*.13,.015,.58)],.020,"metal_warm",cfg["tube_sides"],.012)
        tube_path("R158_SideArmor",[(side*.22,.10,.88),(side*.27,.02,.73),(side*.20,-.08,.61)],.032,"paint_primary",cfg["tube_sides"],.020)
    # Neck and helmet are a tapered wearable shell, not a floating sphere.
    loft("R158_NeckShell",[(-.01,0,.96,.10,.09),(.04,0,1.10,.115,.11)],max(12,cfg["body_sides"]//2),"paint_primary",2.0)
    ellipsoid("R158_HelmetShell",(0,.035,1.18),(.118,.108,.138),"paint_primary",cfg["sphere_seg"],cfg["sphere_ring"])
    ellipsoid("R158_HelmetVisor",(0,-.104,1.205),(.088,.010,.050),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    tube_path("R158_HelmetCrownStripe",[(-.075,-.075,1.285),(0,-.092,1.315),(.075,-.075,1.285)],.012,"accent_emissive",6,.008)
    # R158: shallow side-visible visor lens seated on the shell surface for profile proof.
    # R158: continuous oval visor gasket seated against the helmet side shell.
    visor_ring=[]
    for i in range(24):
        a=TAU*i/24
        visor_ring.append((.158, -.005 + .082*math.cos(a), 1.205 + .054*math.sin(a)))
    for side in (-1,1):
        cylinder_x("R158_VisorPivot",(side*.135,.005,1.19),.017,.025,"metal_warm",cfg["rim_seg"])
    tube_path("R158_HelmetFrame",[(-.075,-.027,1.17),(-.082,-.029,1.205),(0,-.031,1.25),(.082,-.029,1.205),(.075,-.027,1.17)],.008,"metal_warm",cfg["tube_sides"],.006)
    tube_path("R158_ChinGuard",[(-.12,-.12,1.10),(0,-.17,1.065),(.12,-.12,1.10)],.030,"paint_primary",cfg["tube_sides"],.020)
    tube_path("R158_Collar",[(-.17,.01,1.03),(0,.07,1.00),(.17,.01,1.03)],.030,"metal_warm",cfg["tube_sides"],.020)
    # Legs and boots visibly terminate in the pedal shelf.
    triangulated_box("R158_PedalShelf",(0,-.46,.29),(.44,.29,.035),"metal_warm")
    for side in (-1,1):
        tube_path("R158_Thigh",[(side*.14,.10,.58),(side*.17,-.15,.46),(side*.13,-.36,.35)],.078,"paint_secondary",cfg["tube_sides"],.060)
        tube_path("R158_Shin",[(side*.13,-.34,.36),(side*.11,-.49,.33),(side*.095,-.56,.335)],.054,"paint_primary",cfg["tube_sides"],.040)
        ellipsoid("R158_Boot",(side*.095,-.565,.35),(.068,.10,.052),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    ...
    tube_path("R158_SteeringRing",wheel_pts,.022,"paint_primary",cfg["tube_sides"],closed=True)
    cylinder_x("R158_SteeringHub",(0,-.46,.82),.040,.060,"metal_warm",cfg["rim_seg"])
    cylinder_x("R158_SteeringCap",(0,-.46,.82),.016,.066,"paint_secondary",cfg["rim_seg"])
    tube_path("R158_SteeringColumn",[(0,-.46,.82),(0,-.42,.66),(0,-.42,.61)],.020,"accent_emissive",cfg["tube_sides"],.015)
    for side in (-1,1):
        tube_path("R158_UpperArm",[(side*.21,.10,.91),(side*.31,-.03,.84),(side*.23,-.17,.82)],.056,"paint_primary",cfg["tube_sides"],.044)
        tube_path("R158_Forearm",[(side*.23,-.17,.82),(side*.18,-.445,.82)],.047,"paint_secondary",cfg["tube_sides"],.038)
        a=math.radians(30 if side>0 else 150); gx=.17*math.cos(a); gz=.82+.17*math.sin(a)
        hand=ellipsoid("R158_Hand",(gx,-.455,gz),(.039,.030,.047),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"]); hand["grip_angle_deg"]=30 if side>0 else 150
    # R158 class change: replace the isolated arc/rollbar reading with a shallow,
    ...
    # R158: remove the dominant opaque cowl slab; retain only the compact authored lens below.
    ...
    # R158: the review windshield is now a single flush lens with no exposed rail rods.
    # Cowl connection is represented by the lens edge and the two flush mount points only.
    for side in (-1,1):
        fast=ellipsoid("R158_CowlMount",(side*.17,-.59,.64),(.018,.010,.010),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"])
        fast["mount_type"]="integrated_cowl_shell_mount"


def create_cockpit_driver(cfg):
    if cfg["level"] == 0:
        create_cockpit_driver_r158(cfg)
        return
    ...
    # R158: replace exposed inner head sphere with an integrated faceplate/gaiter.
    ellipsoid("HelmetShell",(0,.035,1.18),(.114,.104,.133),"paint_primary",cfg["sphere_seg"],cfg["sphere_ring"])
    ellipsoid("HelmetVisor",(0,-.104,1.205),(.084,.010,.048),"rubber_dark",cfg["sphere_seg"],cfg["sphere_ring"])
    visor_ring=[]
    for i in range(24):
        a=TAU*i/24
    ...
    if cfg["level"] == 0:
        ...
        # R158: low, shallow cowl deflector; top edge remains below the helmet chin.
        ...
        # R158: lens is flush-mounted directly to the cowl; no exposed rail.
        for side in (-1,1):
    ...
    for side in (-1,1):
        ...
        # R158: glove cuff overlaps the forearm and hand, proving a functional grip chain.
        ellipsoid("R158_GloveCuff",(gx,-.445,gz),(.048,.035,.052),"rubber_dark",10,6)
...
def main():
    ...
    pilot=bpy.data.objects.get('LOD0_R158_PilotShell')
    if pilot:
    ...
    # R158: preserve the authored wearable helmet stack; do not collapse it into a head sphere.
    # Lower the complete helmet/visor/chin/collar assembly into the neck socket.
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R158_") and any(k in obj.name for k in ("Helmet", "Visor", "ChinBar", "Collar", "Neck")):
            obj.location.z -= 0.055
    # Make the visor visibly front-facing and continuous with the shell.
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R158_HelmetVisor"):
    ...
    for obj in bpy.data.objects:
        if obj.name.startswith("LOD0_R158_") and any(k in obj.name for k in ("NeckShell", "Collar")):
            obj["pilot_attachment"] = "neck_to_shoulder_bridge"
    out=os.path.abspath(args.output or os.path.join(os.path.dirname(__file__), "hero-kart-v2-R158.blend")); os.makedirs(os.path.dirname(out),exist_ok=True)
