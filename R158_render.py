...
blend=os.path.join(job,'assets/hero-kart-v2/R158/hero-kart-v2-R158.blend')
out=os.path.join(job,'assets/hero-kart-v2/R158/renders'); os.makedirs(out,exist_ok=True)
view='beauty'
if '--view' in sys.argv: view=sys.argv[sys.argv.index('--view')+1]
bpy.ops.wm.open_mainfile(filepath=blend); sc=bpy.context.scene
sc.render.engine='BLENDER_EEVEE'; sc.render.resolution_x=960; sc.render.resolution_y=720; sc.render.resolution_percentage=100
...
sc.render.filepath=os.path.join(out,view+'.png'); bpy.ops.render.render(write_still=True)
print('R158_RENDERED_'+view)
