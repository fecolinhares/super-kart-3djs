...
job=os.path.dirname(os.path.abspath(__file__)); blend=os.path.join(job,'assets/hero-kart-v2/R116/hero-kart-v2-R116.blend'); out=os.path.join(job,'assets/hero-kart-v2/R116/renders'); os.makedirs(out,exist_ok=True)
...
for name,(pos,target,lens) in views.items(): aim(pos,target,lens); sc.render.filepath=os.path.join(out,name+'.png'); bpy.ops.render.render(write_still=True); print('R116_RENDERED',name)
print('R116_RENDER_SET_OK',out)
