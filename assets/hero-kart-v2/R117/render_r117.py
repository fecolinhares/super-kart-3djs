...
job=os.path.dirname(os.path.abspath(__file__)); blend=os.path.join(job,'hero-kart-v2-R117.blend'); out=os.path.join(job,'renders'); os.makedirs(out,exist_ok=True)
...
for name,(pos,target,lens) in views.items(): aim(pos,target,lens); sc.render.filepath=os.path.join(out,name+'.png'); bpy.ops.render.render(write_still=True); print('R117_RENDERED',name)
print('R117_RENDER_SET_OK',out)
