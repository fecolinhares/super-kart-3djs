#!/usr/bin/env python3
"""Build Super Kart Hero V2 R118 from a corrected authored parametric topology.
...
def parse_args():
    ...
    p.add_argument("--revision", default="R118")
...
def create_cockpit_driver(cfg):
    ...
    if cfg["level"] == 0:
        # R118: a low closed visor shell, curved in plan and profile, with a cowl tangent.
        # This is a single authored volume—not a flat plate or a stack of shards.
        visor_sections=[(-.76,0,.77,.145,.145),(-.70,0,.775,.17,.15),(-.61,0,.78,.185,.145),(-.52,0,.785,.18,.13),(-.45,0,.79,.15,.10)]
        visor=loft("KartWindshieldLens",visor_sections,12,"cockpit_glass",2.35)
        bm=bmesh.new(); bm.from_mesh(visor.data); bmesh.ops.triangulate(bm, faces=list(bm.faces)); bmesh.ops.recalc_face_normals(bm, faces=list(bm.faces)); bm.to_mesh(visor.data); bm.free()
        # Four mounts terminate in the cowl and the rear pair follows the shell slope.
        for side in (-1,1):
            tube_path("KartWindshieldAPillar",[(side*.145,-.76,.63),(side*.17,-.68,.67),(side*.18,-.56,.70)],.012,"metal_warm",cfg["tube_sides"],.009)
            tube_path("KartWindshieldTopRail",[(side*.18,-.56,.89),(side*.17,-.68,.92),(side*.145,-.76,.91)],.012,"metal_warm",cfg["tube_sides"],.009)
        tube_path("KartWindshieldCowlRail",[(-.145,-.76,.63),(-.17,-.68,.62),(0,-.61,.615),(.17,-.68,.62),(.145,-.76,.63)],.011,"paint_secondary",cfg["tube_sides"],.009)
        for side in (-1,1):
            fast=ellipsoid("FlushWindshieldFastener",(side*.145,-.76,.63),(.012,.008,.008),"metal_warm",cfg["sphere_seg"],cfg["sphere_ring"])
...
def main():
    ...
    configs={"LOD0":{"level":0,"body_sides":24,"tube_sides":10,"tire_seg":64,"tire_prof":18,"rim_seg":48,
    ...
    scene["generator_contract"] = "R118-authored-v2; no source import; UV-per-mesh; canonical-material-remap; closed tire profiles"
    bpy.context.scene["windshield_width_m"] = .37
    bpy.context.scene["cockpit_width_reference_m"] = .62
    bpy.context.scene["windshield_width_ratio"] = .37/.62
    bpy.context.scene["windshield_mount_count"] = 4
    bpy.context.scene["windshield_rear_edge_y_m"] = -.45
    ...
    out=os.path.abspath(args.output or os.path.join(os.path.dirname(__file__), "hero-kart-v2-R118.blend")); os.makedirs(os.path.dirname(out),exist_ok=True)
    bpy.ops.wm.save_as_mainfile(filepath=out,compress=True)
    report={"revision":args.revision,"blender":bpy.app.version_string,"blend":out,
            "windshield_mount_count":4,"windshield_width_m":.37,
            "cockpit_width_reference_m":.62,"windshield_width_ratio":.37/.62,
            "windshield_rear_edge_y_m":-.45,"cockpit_rim_rear_y_m":.50,
