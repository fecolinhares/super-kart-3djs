...
# RODAS
DF,DR,WDF,WDR=0.440,0.580,0.240,0.320; TF,TR=0.78,0.96; xf,xr=0.70,-0.60
...
box("C_Floor",-0.86,0.62, -0.26,0.26, 0.13,0.24)
# SIDEPODS: parte dianteira BAIXA (pernas) + parte traseira ALTA (flanqueia o piloto) — o corte do gate
box("SP_Lf", 0.02,0.40,  0.26,0.56, 0.15,0.33)
box("SP_Rf", 0.02,0.40, -0.56,-0.26, 0.15,0.33)
box("SP_Lr",-0.66,0.02,  0.26,0.58, 0.15,0.58)
box("SP_Rr",-0.66,0.02, -0.58,-0.26, 0.15,0.58)
...
cil("B_Top",1.03,0,0.42,0.075,1.24)
cil("B_ArmL",0.86, 0.58,0.28,0.075,0.44,(0,math.radians(90),0))
cil("B_ArmR",0.86,-0.58,0.28,0.075,0.44,(0,math.radians(90),0))
...
cil("P_ArmL",-0.34, 0.22,0.52, 0.055,0.44, rot=(0,math.radians(58),0))
cil("P_ArmR",-0.34,-0.22,0.52, 0.055,0.44, rot=(0,math.radians(58),0))
toro("P_Wheel",(0.10,0,0.47),0.135,0.028, rot=(0,math.radians(62),0))
cil("P_Col",0.26,0,0.40,0.028,0.42, rot=(0,math.radians(55),0))
for sy,sn in ((1,"L"),(-1,"R")):
    box("P_Thigh"+sn,-0.20,0.30, sy*0.10-0.09, sy*0.10+0.09, 0.26,0.42)
    box("P_Shin"+sn, 0.26,0.38, sy*0.10-0.07, sy*0.10+0.07, 0.13,0.38, rot=(0,math.radians(48),0))
    box("P_Boot"+sn, 0.38,0.50, sy*0.10-0.08, sy*0.10+0.08, 0.13,0.21)
box("P_Hips",-0.52,-0.20, -0.21,0.21, 0.24,0.46)
esf("P_Head",(-0.44,0,0.80),0.215,0.215,0.215)
box("P_Visor",-0.63,-0.55, -0.15,0.15, 0.76,0.88)
box("P_Seat",-0.90,-0.76, -0.23,0.23, 0.24,0.76)
# TRASEIRA: motor + 3 escapes + eixo + para-choque em U vazado
box("R_Motor",-1.02,-0.72, -0.32,0.32, 0.14,0.52)
cil("R_Exh1",-1.10,0,0.46,0.075,0.30); cil("R_Exh2",-1.10,0.19,0.46,0.075,0.30); cil("R_Exh3",-1.10,-0.19,0.46,0.075,0.30)
cil("R_Axle",-0.60,0,DR/2,0.045,0.98,(0,math.radians(90),0))
cil("R_BTop",-1.14,0,0.30,0.06,1.02)
cil("R_BArmL",-0.98, 0.48,0.22,0.06,0.34,(0,math.radians(90),0))
cil("R_BArmR",-0.98,-0.48,0.22,0.06,0.34,(0,math.radians(90),0))
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B010.blend')
print('###B010### ok')
