...
cil("AX_F",0.72,0,R/2,0.045,0.90,(0,math.radians(90),0)); suave("AX_F",1)
cil("W_RL",-0.65,0.53,R,R,0.210); cil("W_RR",-0.65,-0.53,R,R,0.210)
cil("AX_R",-0.65,0,R/2,0.045,1.10,(0,math.radians(90),0)); suave("AX_R",1)
for n in ("W_FL","W_FR","W_RL","W_RR"): suave(n,1)
# ===== BICO: ELIPSOIDE AFUNILADO (curvo, nao escada) =====
esf("C_Nose",(0.62,0,0.30),0.44,0.22,0.16); suave("C_Nose",2)
...
for sy,sn in ((1,"L"),(-1,"R")):
    esf("SP_"+sn,(-0.22, sy*0.30, 0.34), 0.54,0.17,0.15); suave("SP_"+sn,2)
...
box("P_Torso",-0.66,-0.46, -0.17,0.17, 0.34,0.58, rot=(0,math.radians(-40),0)); suave("P_Torso",1)
cil("P_Neck",-0.60,0,0.52,0.085,0.14,(0,0,0)); suave("P_Neck",1)
esf("P_Head",(-0.60,0,0.60),0.120,0.120,0.120); suave("P_Head",1)
for sy,sn in ((1,"L"),(-1,"R")):
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B019.blend')
print('###B017### ok')
