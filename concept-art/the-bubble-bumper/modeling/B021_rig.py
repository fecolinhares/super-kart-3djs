...
cil("HUB_FL",0.72, 0.505,R,R*0.45,0.06); cil("HUB_FR",0.72,-0.505,R,R*0.45,0.06)
cil("AX_F",0.72,0,R/2,0.035,0.90,(0,math.radians(90),0)); suave("AX_F",0)
cil("W_RL",-0.65,0.53,R,R,0.210); cil("W_RR",-0.65,-0.53,R,R,0.210)
cil("HUB_RL",-0.65,0.635,R,R*0.45,0.06); cil("HUB_RR",-0.65,-0.635,R,R*0.45,0.06)
cil("AX_R",-0.65,0,R/2,0.035,1.10,(0,math.radians(90),0)); suave("AX_R",0)
for n in ("W_FL","W_FR","W_RL","W_RR"): suave(n,1)
# ===== BICO: ELIPSOIDE AFUNILADO (curvo, nao escada) =====
esf("C_Nose",(0.60,0,0.30),0.31,0.24,0.17); suave("C_Nose",2)
...
for sy,sn in ((1,"L"),(-1,"R")):
    esf("SP_"+sn,(-0.24, sy*0.30, 0.32), 0.43,0.17,0.13); suave("SP_"+sn,2)
...
cil("R_ExhL",-1.02, 0.13,0.44,0.055,0.26,(0,math.radians(72),0)); cil("R_ExhR",-1.02,-0.13,0.44,0.055,0.26,(0,math.radians(72),0))
cil("R_ExhC",-1.04,0,0.48,0.060,0.26,(0,math.radians(72),0))
...
box("P_Hips",-0.52,-0.24, -0.19,0.19, 0.22,0.42, rot=(0,math.radians(-8),0)); suave("P_Hips",1)
box("P_Torso",-0.66,-0.44, -0.17,0.17, 0.30,0.62, rot=(0,math.radians(-45),0)); suave("P_Torso",1)
cil("P_Neck",-0.54,0,0.46,0.080,0.14,(0,math.radians(-30),0)); suave("P_Neck",0)
esf("P_Head",(-0.50,0,0.54),0.105,0.105,0.105); suave("P_Head",1)
for sy,sn in ((1,"L"),(-1,"R")):
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B021.blend')
print('###B017### ok')
