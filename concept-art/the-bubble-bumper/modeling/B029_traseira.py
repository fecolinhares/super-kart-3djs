...
D=0.407; R=D/2
cil("W_FL",0.72, 0.50,R,R,0.190); cil("W_FR",0.72,-0.50,R,R,0.190)
cil("HUB_FL",0.72, 0.585,R,R*0.40,0.07); cil("HUB_FR",0.72,-0.585,R,R*0.40,0.07)
cil("AX_F",0.72,0,R/2,0.032,0.72,(0,math.radians(90),0)); suave("AX_F",0)
cil("W_RL",-0.65,0.62,R,R,0.250); cil("W_RR",-0.65,-0.62,R,R,0.250)
cil("HUB_RL",-0.65,0.735,R,R*0.40,0.07); cil("HUB_RR",-0.65,-0.735,R,R*0.40,0.07)
...
box("C_Nose",0.40,0.84, -0.17,0.17, 0.22,0.38); suave("C_Nose",2); suave("C_Nose",2)
...
for sy,sn in ((1,"L"),(-1,"R")):
    esf("SP_"+sn,(-0.24, sy*0.30, 0.26), 0.40,0.15,0.10); suave("SP_"+sn,2)
# ===== BUMPER DIANTEIRO: TORO EM C CONTINUO (corta a metade traseira com boolean) =====
bpy.ops.mesh.primitive_torus_add(major_radius=0.50, minor_radius=0.090, major_segments=40, minor_segments=14,
                                 location=(0.58,0,0.20))
...
bpy.ops.mesh.primitive_torus_add(major_radius=0.52, minor_radius=0.095, major_segments=36, minor_segments=12,
                                 location=(-0.56,0,0.20))
...
esf("R_Motor",(-0.88,0,0.40),0.34,0.30,0.26); suave("R_Motor",2)
...
cil("R_ExhC",-1.06,0,0.50,0.075,0.30,(0,math.radians(72),0))
box("P_Seat",-0.84,-0.68, -0.24,0.24, 0.26,0.62, rot=(0,math.radians(-18),0))
box("R_Wing",-1.16,-1.04, -0.52,0.52, 0.78,0.90); suave("R_Wing",2)
box("R_WingPost",-1.12,-1.06, -0.07,0.07, 0.44,0.80)
cil("R_HoleC",-1.08,0,0.48,0.042,0.10,(0,math.radians(72),0))
# ===== PILOTO ARTICULADO COM PESCOCO =====
box("P_Hips",-0.52,-0.24, -0.19,0.19, 0.24,0.44, rot=(0,math.radians(-8),0)); suave("P_Hips",1)
box("P_Torso",-0.62,-0.40, -0.19,0.19, 0.36,0.94, rot=(0,math.radians(-14),0))
box("P_Shoulder",-0.50,-0.30, -0.27,0.27, 0.78,0.90); suave("P_Torso",1)
capsula("P_Neck",-0.60,-0.48, 0,0.92,0.085); suave("P_Neck",0)
esf("P_Head",(-0.52,0,1.10),0.150,0.150,0.150); suave("P_Head",1)
for sy,sn in ((1,"L"),(-1,"R")):
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B029.blend')
print('###B017### ok')
