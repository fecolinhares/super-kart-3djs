...
box("P_Seat",-0.56,-0.40, -0.24,0.24, 0.26,0.62, rot=(0,math.radians(-18),0))
box("R_Wing",-1.16,-1.04, -0.72,0.72, 0.80,0.92); suave("R_Wing",2); suave("R_Wing",2)
box("R_WingPost",-1.12,-1.06, -0.07,0.07, 0.44,0.82)
for _sy in (1,-1):
    cil("R_End"+("L" if _sy>0 else "R"),-1.10, 0.72*_sy,0.86,0.150,0.045,(0,math.radians(90),0))
...
box("P_Torso",-0.34,-0.12, -0.19,0.19, 0.36,0.94, rot=(0,math.radians(-14),0))
box("P_Shoulder",-0.50,-0.30, -0.27,0.27, 0.78,0.90); suave("P_Torso",1)
capsula("P_Neck",-0.32,-0.20, 0,0.92,0.085); suave("P_Neck",0)
esf("P_Head",(-0.24,0,1.10),0.150,0.150,0.150); suave("P_Head",1)
for sy,sn in ((1,"L"),(-1,"R")):
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B031.blend')
print('###B017### ok')
