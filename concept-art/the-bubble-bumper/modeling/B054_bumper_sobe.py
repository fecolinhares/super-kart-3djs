...
D=0.407; R=D/2
cil("W_FL",0.72, 0.47,R,R,0.180); cil("W_FR",0.72,-0.47,R,R,0.180)
...
for sy,sn in ((1,"L"),(-1,"R")):
    esf("SP_"+sn,(-0.30, sy*0.40, 0.24), 0.50,0.15,0.085); suave("SP_"+sn,2)
# ===== BUMPER DIANTEIRO: TORO EM C CONTINUO (corta a metade traseira com boolean) =====
bpy.ops.mesh.primitive_torus_add(major_radius=0.66, minor_radius=0.085, major_segments=40, minor_segments=14,
                                 location=(0.58,0,0.60))
...
bpy.ops.mesh.primitive_torus_add(major_radius=0.46, minor_radius=0.070, major_segments=36, minor_segments=12,
                                 location=(-0.56,0,0.22))
...
box("R_Wing",-1.16,-1.04, -0.80,0.80, 0.78,0.94); suave("R_Wing",2)
...
esf("P_Head",(-0.24,0,1.09),0.176,0.176,0.166); suave("P_Head",1)
for sy,sn in ((1,"L"),(-1,"R")):
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B054.blend')
print('###B017### ok')
