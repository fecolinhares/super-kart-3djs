...
D=0.53; R=D/2
cil("W_FL",0.72, 0.50,R,R,0.180); cil("W_FR",0.72,-0.50,R,R,0.180)
...
# DOMO QUEBRADO EM MECANICA LEGIVEL (vision: 'domo monolitico'): duas carenagens
# laterais com VAO CENTRAL expondo motor e escapes, como o concept
for sf in (1,-1):
    y0,y1=sorted((sf*0.34, sf*0.70))
    box("R_Fair"+("L" if sf>0 else "R"), -1.22,-0.60, y0,y1, 0.26,0.62); suave("R_Fair"+("L" if sf>0 else "R"),1)
# mola visivel no vao central (elemento do concept)
cil("R_Spring",-0.92,0,0.44,0.055,0.26,(0,math.radians(90),0)); suave("R_Spring",1); suave("R_Fairing",1)
...
box("R_Wing",-1.18,-1.02, -0.19,0.19, 1.11,1.25); suave("R_Wing",2)
box("R_WingPost",-1.12,-1.06, -0.08,0.08, 0.36,1.16)
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B104.blend')
print('###B017### ok')
