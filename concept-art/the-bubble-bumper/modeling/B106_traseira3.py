...
for sf in (1,-1):
    ...
    box("R_Pod"+("L" if sf>0 else "R"), -1.12,-0.66, y0,y1, 0.24,0.62); suave("R_Pod"+("L" if sf>0 else "R"),1)
box("R_Motor",-1.02,-0.74, -0.24,0.24, 0.28,0.60); suave("R_Motor",1)   # bloco central, com AR ate os pods; suave("R_Fairing",1)
...
esf("P_Head",(-0.24,0,0.88),0.160,0.160,0.150)
box("R_RollHoop",-0.44,-0.34, -0.20,0.20, 0.84,1.20); suave("R_RollHoop",1); suave("P_Head",1)
for sy,sn in ((1,"L"),(-1,"R")):
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B106.blend')
print('###B017### ok')
