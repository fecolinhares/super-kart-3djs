...
bpy.ops.mesh.primitive_torus_add(major_radius=0.34, minor_radius=0.048, major_segments=40, minor_segments=14,
...
suave("B_Front",1)
# 2 CINTAS AMARELAS do para-choque dianteiro (o vision: "para-choque azul com 2 cintas amarelas")
for sf in (1,-1):
    box("B_Strap"+("L" if sf>0 else "R"), 0.44,0.60, sf*0.26,sf*0.40, 0.40,0.50); suave("B_Strap"+("L" if sf>0 else "R"),1)
...
for sf in (1,-1):
    box("R_EndP"+("L" if sf>0 else "R"),-1.235,-1.03, sf*0.50,sf*0.56, 0.50,0.80); suave("R_EndP"+("L" if sf>0 else "R"),1)
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B116.blend')
print('###B017### ok')
