...
D=0.53; R=D/2
RF=R*0.42/0.84; cil("W_FL",0.72, 0.52,RF*0.9412*0.968,RF,0.150); suave("W_FL",1)
cil("W_FR",0.72,-0.52,RF*0.9412*0.968,RF,0.150); suave("W_FR",1)
...
RR=R*0.50; RR=R*0.50/0.84; cil("W_RL",-0.65,0.58,RR*0.9412*0.968,RR,0.240); suave("W_RL",1)
cil("W_RR",-0.65,-0.58,RR*0.9412*0.968,RR,0.240); suave("W_RR",1)
...
for sy,sn in ((1,"L"),(-1,"R")):
    ...
    capsula("SP_"+sn, -0.86, 0.04, sy*0.44, 0.455, 0.185)
    ...
    box("SPM_"+sn, -0.90, 0.12, *sorted((sy*0.26, sy*0.62)), 0.50,0.53)
...
box("B_Caren",0.58,0.96, -0.60,0.60, 0.24,0.50); suave("B_Caren",2)
box("B_Nose",0.70,1.02, -0.20,0.20, 0.28,0.46); suave("B_Nose",1)
...
box("R_Wing",-1.22,-1.05, -0.50,0.50, 0.52,0.62); suave("R_Wing",2)
...
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B125.blend')
print('###B017### ok')
