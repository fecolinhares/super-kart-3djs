...
for sy,sn in ((1,"L"),(-1,"R")):
    ...
    capsula("SP_"+sn, -0.84, 0.06, sy*0.44, 0.185, 0.145)
...
# ESCALA MESTRE DO PILOTO (vision: 'Godzilla 2x maior, isso sozinho destroi FRONT/SIDE/REAR')
_ps=[o for o in bpy.data.objects if o.name.startswith("P_")]
if _ps:
    import mathutils
    _c=mathutils.Vector((0,0,0))
    for _o in _ps: _c=_c+_o.matrix_world.translation
    _c=_c/len(_ps)
    bpy.ops.object.select_all(action="DESELECT")
    for _o in _ps:
        _o.select_set(True)
        _o.location=_c+(_o.location-_c)*0.70
        _o.scale=(0.70,0.70,0.70)
    bpy.ops.object.transform_apply(location=True, scale=True)
    _zs=[(_o.matrix_world@_v.co).z for _o in _ps for _v in _o.data.vertices]
    print("###ESCALA### %d pecas | piloto z[%.3f ; %.3f]"%(len(_ps), min(_zs), max(_zs)))
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B134.blend')
print('###B017### ok')
