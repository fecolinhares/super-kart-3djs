...
cil("R_Motor",-0.86,-0.60, 0,-0.16, 0.09,(0,math.radians(90),0)); suave("R_Motor",1)
box("R_Block",-0.84,-0.62, -0.20,0.20, 0.10,0.30); suave("R_Block",1)
for _i,_y in enumerate((-0.13,0.0,0.13)):
    cil("R_Exh%d"%_i,-1.06,-0.66, _y,0, 0.042,(0,math.radians(90),0)); suave("R_Exh%d"%_i,1)
print("###MOTOR### compacto + 3 escapes cilindricos")

# RODAS REAIS DO CONCEPT (causa nº1 do vision): slicks PRETOS, traseira 2x a dianteira, NO CHAO
def roda(n, x, y, D, larg):
    r=D/2.0
    cil(n, x-0.001, x+0.001, y, r*0.9412, r, (0,math.radians(90),0))  # 0.9412 = compensacao SubD (regra 97)
    suave(n,1)
_axf, _axr = 0.80, -0.80      # eixos
for _sn,_sy in (("L",1),("R",-1)):
    roda("W_F"+_sn, _axf, _sy*0.520, 0.30, 0.17)   # dianteira: D=0,30 (pequena, estreita)
    roda("W_R"+_sn, _axr, _sy*0.560, 0.46, 0.28)   # traseira: D=0,46 (grande, LARGA) = 2x
print("###RODAS### 4 rodas: diant 0,30x0,17 / tras 0,46x0,28")
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B147.blend')
print('###B017### ok')
