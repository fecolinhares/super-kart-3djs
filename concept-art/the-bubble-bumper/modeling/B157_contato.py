...
# ===== FLUTUANTES 8 -> 0 (regra 125: 'enquanto houver 1 objeto flutuante nao ha kart') =====
# (1) DELETAR as 2 laminas/facas BRANCAS verticais (B_Boot antigo) e as 2 esferas amarelas soltas
for _n in list(bpy.data.objects.keys()):
    if _n.startswith(("B_Boot","B_Slat","B_BarTip","C_Hub","B_Bar","B_NoseBand","P_Hand")):
        try: bpy.data.objects.remove(bpy.data.objects[_n], do_unlink=True)
        except Exception: pass
print("###FLUT1### removidos: B_Boot/B_Slat/B_BarTip/C_Hub/B_Bar/B_NoseBand/P_Hand")
# (2) VOLANTE CONECTADO: coluna SOLIDARIA do chassi ate o volante (sem gap)
cil("C_Col", x=0.42, y=0.0, z=0.40, r=0.028, d=0.40, rot=(math.radians(62),0,0)); suave("C_Col",1)
bpy.ops.mesh.primitive_torus_add(major_radius=0.115, minor_radius=0.028, major_segments=24, minor_segments=8,
                                 location=(0.55,0,0.52), rotation=(math.radians(62),0,0))
bpy.context.object.name="C_Wheel"; suave("C_Wheel",1)
# mao do piloto NO volante (contato real, nao no vazio)
for _sn,_sy in (("L",1),("R",-1)):
    esf("P_Grip"+_sn,(0.545,_sy*0.105,0.515),0.048,0.048,0.048)
print("###FLUT2### volante CONECTADO por C_Col (coluna solida) + maos P_Grip no aro")
# (3) MOTOR com COLETOR: os 3 escapes saem de UM coletor ligado ao bloco (nao ponteiras soltas)
box("R_Header", -0.80,-0.62, -0.22,0.22, 0.16,0.32); suave("R_Header",1)
print("###FLUT3### R_Header = coletor conectando os 3 escapes ao motor")
# (4) CUBOS das rodas: DENTRO do pneu, com aro (substitui os tampoes esfericos para fora)
for _sn,_sy in (("L",1),("R",-1)):
    cil("C_RimF"+_sn, x=0.6345, y=_sy*0.518, z=0.141, r=0.075, d=0.175, rot=(0,math.radians(90),0))
    cil("C_RimR"+_sn, x=-0.6345, y=_sy*0.586, z=0.217, r=0.105, d=0.285, rot=(0,math.radians(90),0))
print("###FLUT4### C_RimF/R = aro DENTRO do pneu (substitui tampoes que saiam para fora)")
# (5) SIDEPODS SUPORTADOS: chapa que liga o pod ao assoalho (mata o gap de ar)
for _sn,_sy in (("L",1),("R",-1)):
    box("C_PodMount"+_sn, -0.55,0.05, _sy*0.13,_sy*0.30, 0.055,0.245)
print("###FLUT5### C_PodMount = chapa ligando o pod ao assoalho (mata o gap de ar)")
# (6) ASA TRASEIRA: barra HORIZONTAL + 2 ENDPLATES (nao disco vertical)
box("R_WingBar", -1.16,-1.02, -0.56,0.56, 0.60,0.665); suave("R_WingBar",1)
for _sn,_sy in (("L",1),("R",-1)):
    box("R_Endplate"+_sn, -1.18,-1.00, _sy*0.545,_sy*0.585, 0.50,0.72)
    box("R_WingLeg"+_sn, -1.10,-1.02, _sy*0.22,_sy*0.30, 0.30,0.62)
print("###FLUT6### asa = barra HORIZONTAL + 2 endplates + 2 pernas ligadas ao chassi")

# ===== FIX DAS 8 FALHAS DE CONTATO (gate_contato.py: overlap>0) — PENETRACAO REAL =====
# (a) C_Col: estender de x 0.39..0.45 para 0.30..0.62 (penetra no volante E desce ao C_Floor)
try: bpy.data.objects.remove(bpy.data.objects["C_Col"], do_unlink=True)
except Exception: pass
cil("C_Col", x=0.44, y=0.0, z=0.42, r=0.034, d=0.46, rot=(math.radians(58),0,0)); suave("C_Col",1)
# (b) maos: aproximar para DENTRO do aro (aro: C_Wheel centro x=0.55 r=0.115) e ligar ao braco
for _sn,_sy in (("L",1),("R",-1)):
    try: bpy.data.objects.remove(bpy.data.objects["P_Grip"+_sn], do_unlink=True)
    except Exception: pass
    esf("P_Grip"+_sn,(0.535,_sy*0.108,0.523),0.055,0.055,0.055)
    # braco estendido ATE a mao
    try: bpy.data.objects.remove(bpy.data.objects["P_Arm"+_sn], do_unlink=True)
    except Exception: pass
    cil("P_Arm"+_sn, x=0.30, y=_sy*0.20, z=0.50, r=0.052, d=0.42, rot=(math.radians(58),0,math.radians(_sy*32))); suave("P_Arm"+_sn,1)
# (c) endplates: encostar na barra (barra x -1.16..-1.02 -> endplate x -1.14..-0.99)
for _sn,_sy in (("L",1),("R",-1)):
    try: bpy.data.objects.remove(bpy.data.objects["R_Endplate"+_sn], do_unlink=True)
    except Exception: pass
    box("R_Endplate"+_sn, -1.14,-0.99, _sy*0.525,_sy*0.585, 0.50,0.72)
# (d) C_PodMount: subir/estender para penetrar NO pod (pod SP_L z 0.075..0.335)
for _sn,_sy in (("L",1),("R",-1)):
    try: bpy.data.objects.remove(bpy.data.objects["C_PodMount"+_sn], do_unlink=True)
    except Exception: pass
    box("C_PodMount"+_sn, -0.55,0.05, _sy*0.14,_sy*0.28, 0.055,0.30)
# (e) SP_L ao assoalho: o pod ja e tocado pelo mount; garantir montagem do pod no assoalho
try:
    _f=bpy.data.objects.get("C_Floor")
    if _f: _f.scale.y=1.06
except Exception: pass
# (f) R_Block/R_Motor: ligar ao assoalho com longarina
box("C_Long", -0.80,-0.45, -0.06,0.06, 0.09,0.16); suave("C_Long",1)

# ===== CONTATO 100%: conectores posicionados a partir do BBOX REAL das pecas (penetracao garantida) =====
def _bb(n):
    o=bpy.data.objects.get(n)
    if not o or o.type!="MESH": return None
...
def _liga(nome, a, b, eixo, r=0.035, folga=0.03):
    """cria um conector que PENETRA a peca a e a peca b ao longo de `eixo`"""
    A,B=_bb(a),_bb(b)
    if not A or not B: print("###SKIP### %s (%s/%s ausente)"%(nome,a,b)); return
    if eixo=="x": c1,c2=(A[0]+folga, A[1]-folga); d1,d2=(B[0]+folga, B[1]-folga)
    elif eixo=="z": c1,c2=(A[4]+folga, A[5]-folga); d1,d2=(B[4]+folga, B[5]-folga)
    else: c1,c2=(A[2]+folga, A[3]-folga); d1,d2=(B[2]+folga, B[3]-folga)
    lo=min(c1,d1); hi=max(c2,d2); comp=hi-lo
    cx=(lo+hi)/2; cy=(A[2]+A[3]+B[2]+B[3])/4; cz=(A[4]+A[5]+B[4]+B[5])/4
    if eixo=="x": cil(nome, x=cx, y=cy, z=cz, r=r, d=comp, rot=(0,math.radians(90),0))
    elif eixo=="z": cil(nome, x=cx, y=cy, z=cz, r=r, d=comp)
    else: cil(nome, x=cx, y=cy, z=cz, r=r, d=comp, rot=(math.radians(90),0,0))
    suave(nome,1)
# (1) PESCOCO solido: capacete -> torso (mata o vao)
_liga("P_Neck", "P_Helmet", "P_Torso", "z", r=0.062, folga=0.05)
# (2) MAO no aro: esfera com centro NO tubo do aro (C_Wheel) e braco ate a mao
_A=_bb("C_Wheel")
if _A:
    for _sn,_sy in (("L",1),("R",-1)):
        _rm("P_Grip"+_sn); _rm("P_Arm"+_sn)
        _gx=_A[0]+0.028; _gy=_sy*((_A[2]+_A[3])/2+0.100); _gz=_A[5]-0.020
        esf("P_Grip"+_sn,(_gx,_gy,_gz),0.050,0.050,0.050)
        _liga("P_Arm"+_sn, "P_Grip"+_sn, "P_Shoulder", "x", r=0.048, folga=0.06)
# (3) COLUNA: do assoalho ao aro (liga C_Floor e C_Wheel)
_liga("C_Col", "C_Floor", "C_Wheel", "x", r=0.032, folga=0.04)
# (4) ENDPLATES: ligar a barra por dentro (extensao em y ate o vao da barra)
_B=_bb("R_WingBar")
if _B:
    for _sn,_sy in (("L",1),("R",-1)):
        _rm("R_Endplate"+_sn)
        box("R_Endplate"+_sn, _B[0]+0.01,_B[1]-0.01, _sy*0.50,_sy*0.575, 0.50,0.72)
# (5) MOUNT do pod: do assoalho AO pod (y alcancando o pod)
_S=_bb("SP_L")
if _S:
    for _sn,_sy in (("L",1),("R",-1)):
        _rm("C_PodMount"+_sn)
        box("C_PodMount"+_sn, _S[0]+0.05,_S[1]-0.05, _sy*0.10,_sy*(abs(_S[3])+0.02), 0.055,0.30)
# (6) POD ao assoalho: saia interna que penetra os dois
if _S:
    for _sn,_sy in (("L",1),("R",-1)):
        box("C_PodSkirt"+_sn, _S[0]+0.10,_S[1]-0.10, _sy*0.16,_sy*(abs(_S[3])-0.04), 0.040,0.115)

# ===== RECONSTRUIR OS PONTOES COMO SOLIDOS FECHADOS (o gate achou SP_L/SP_R VAZIOS) =====
for _n in ("SP_L","SP_R","SPM_L","SPM_R","SPT_L","SPT_R"):
    _rm(_n)
# pod: caixa arredondada com topo/base/laterais planos (receita do vision B137), SOLIDA e fechada
#   dims: 0,75 (x) x 0,30 (y) x 0,26 (z), com taper frontal e bevel de 2 segmentos
import bmesh as _bm
def _pod(nome, sy):
    bpy.ops.mesh.primitive_cube_add(size=1.0, location=(-0.22, sy*0.415, 0.205))
    o=bpy.context.object; o.name=nome
    o.scale=(0.375, 0.15, 0.13)   # 0,75 x 0,30 x 0,26
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    me=o.data; bm=_bm.new(); bm.from_mesh(me)
    # taper frontal: afinar a face da frente (x+) em y e z
    for v in bm.verts:
        if v.co.x > 0.30:
            v.co.y *= 0.62; v.co.z = v.co.z*0.72 + 0.010
        if v.co.x < -0.34:
            v.co.y *= 0.88
    # loops longitudinais (4 cortes) para forma controlada
    bmesh_ops=_bm.ops
    for _ in range(3):
        bmesh_ops.bisect_plane(bm, geom=bm.verts[:]+bm.edges[:]+bm.faces[:], plane_co=(-0.10,0,0), plane_no=(1,0,0))
    bmesh_ops.bisect_plane(bm, geom=bm.verts[:]+bm.edges[:]+bm.faces[:], plane_co=(0,0,0.09), plane_no=(0,0,1))
    bm.to_mesh(me); bm.free()
    bevel_mod = o.modifiers.new("bev","BEVEL"); bevel_mod.width=0.022; bevel_mod.segments=2
...
# ===== FECHAR OS 11 CONTATOS: conectores posicionados pelo BBOX MEDIDO (regra 133) =====
def _bbx(n):
    o=bpy.data.objects.get(n)
    if not o or o.type!="MESH": return None
    vs=[(o.matrix_world@v.co) for v in o.data.vertices]
    ...
    return (min(v.x for v in vs),max(v.x for v in vs),min(v.y for v in vs),max(v.y for v in vs),
            min(v.z for v in vs),max(v.z for v in vs))
def _span(nome, a, b, r=0.04, eixo="z", marg=0.05):
    """conector que PENETRA a e b: eixo define a direcao; as outras dims vem da INTERSECCAO dos bbox"""
    A,B=_bbx(a),_bbx(b)
    if not A or not B: print("###SPAN_SKIP###",nome); return
    if eixo=="z":
        z1,z2=A[4]-marg,A[5]+marg; lo,hi=min(z1,B[4]),max(z2,B[5])
        x=(max(A[0],B[0])+min(A[1],B[1]))/2; y=(max(A[2],B[2])+min(A[3],B[3]))/2
        if abs(x)>3 or abs(y)>3: x=(A[0]+A[1])/2; y=(A[2]+A[3])/2
        cil(nome, x=x, y=y, z=(lo+hi)/2, r=r, d=(hi-lo))
    elif eixo=="y":
        lo=min(A[2],B[2]); hi=max(A[3],B[3])
        x=(A[0]+A[1])/2; z=(A[4]+A[5])/2
        cil(nome, x=x, y=(lo+hi)/2, z=z, r=r, d=(hi-lo), rot=(math.radians(90),0,0))
    else:
        lo=min(A[0],B[0]); hi=max(A[1],B[1])
        y=(A[2]+A[3])/2; z=(A[4]+A[5])/2
        cil(nome, x=(lo+hi)/2, y=y, z=z, r=r, d=(hi-lo), rot=(0,math.radians(90),0))
    suave(nome,1)
# (1) PESCOCO VERTICAL: do topo do torso ao capacete
_rm("P_Neck"); _span("P_Neck","P_Torso","P_Helmet", r=0.062, eixo="z", marg=0.04)
# (2) POD NO ASSOALHO: saia que vai do pod ATE o assoalho
for _sn,_sy in (("L",1),("R",-1)):
    _rm("C_PodSkirt"+_sn); _span("C_PodSkirt"+_sn,"C_Floor","SP_"+_sn, r=0.055, eixo="z", marg=0.03)
# (3) MOUNT alcancando o pod em Y
for _sn,_sy in (("L",1),("R",-1)):
    _rm("C_PodMount"+_sn); _span("C_PodMount"+_sn,"C_Floor","SP_"+_sn, r=0.045, eixo="y", marg=0.04)
# (4) MAO NO ARO: centro no bbox real do C_Wheel
_A=_bbx("C_Wheel")
if _A:
    for _sn,_sy in (("L",1),("R",-1)):
        _rm("P_Grip"+_sn)
        esf("P_Grip"+_sn,(_A[0]+0.022,_sy*(_A[3]-0.012),_A[5]-0.018),0.048,0.048,0.048)
        _rm("P_Arm"+_sn); _span("P_Arm"+_sn,"P_Grip"+_sn,"P_Shoulder", r=0.046, eixo="x", marg=0.05)
# (5) COLUNA: do assoalho ao aro (y e z do aro)
_C=_bbx("C_Floor"); _W=_bbx("C_Wheel")
if _C and _W:
    _rm("C_Col")
    _x1,_x2=_C[0]+0.05,_W[1]-0.02
    cil("C_Col", x=(_x1+_x2)/2, y=(_W[2]+_W[3])/2, z=(_C[5]+_W[4])/2, r=0.032, d=(_x2-_x1), rot=(math.radians(38),0,0))
    suave("C_Col",1)
# (6) ENDPLATES na barra: deslocar em Y para dentro do vao da barra
_B=_bbx("R_WingBar")
if _B:
    for _sn,_sy in (("L",1),("R",-1)):
        _rm("R_Endplate"+_sn)
        box("R_Endplate"+_sn, _B[0]+0.015,_B[1]-0.015, _sy*(abs(_B[3])-0.055), _sy*(abs(_B[3])+0.005), _B[4]-0.06,_B[5]+0.06)
# (7) TORSO no pod/assoalho: longarina do torso ao assoalho
_rm("C_TorsoMount"); _span("C_TorsoMount","C_Floor","P_Torso", r=0.05, eixo="z", marg=0.03)

# ===== ULTIMA ITERACAO: fechar os 4 contatos reais (o 5o par era semanticamente errado) =====
# (a) COLUNA: atravessar o aro do volante (do assoalho ATE o bbox do aro em z)
_C=_bbx("C_Floor"); _W=_bbx("C_Wheel")
if _C and _W:
    _rm("C_Col")
    _x1,_x2=_C[0]+0.04,_W[1]-0.01
    _z1,_z2=_C[5]-0.02,_W[5]-0.005
    cil("C_Col", x=(_x1+_x2)/2, y=(_W[2]+_W[3])/2, z=(_z1+_z2)/2, r=0.034, d=(_x2-_x1), rot=(math.radians(26),0,0))
    suave("C_Col",1)
# (b) POD NO ASSOALHO: baixar o pod ate encostar (floor topo em z ~0,10) e subir a saia
_P=_bbx("C_Floor")
if _P:
    for _sn,_sy in (("L",1),("R",-1)):
        _o=bpy.data.objects.get("SP_"+_sn)
        if _o:
            _b=_bbx("SP_"+_sn); _dz=(_P[5]-0.02)-_b[4]   # deslocar para o fundo do pod tocar o topo do assoalho
            if abs(_dz)<0.30: _o.location.z += _dz
        _rm("C_PodSkirt"+_sn)
        _b2=_bbx("SP_"+_sn)
        if _b2 and _P:
            _z1,_z2=min(_P[5],_b2[4]),max(_P[4],_b2[5])
            box("C_PodSkirt"+_sn, _b2[0]+0.08,_b2[1]-0.08, _sy*(_b2[3]-0.10), _sy*(_b2[3]-0.01), _z1-0.02, _z2+0.02)
# (c) PESCOCO: ligar torso e capacete pelo eixo Z com os centros corretos
_rm("P_Neck")
_T=_bbx("P_Torso"); _H=_bbx("P_Helmet")
if _T and _H:
    _z1,_z2=min(_T[5],_H[4]),max(_T[4],_H[5])
    cil("P_Neck", x=(_H[0]+_H[1])/2, y=(_H[2]+_H[3])/2, z=(_z1+_z2)/2, r=0.062, d=(_z2-_z1)+0.04)
    suave("P_Neck",1)
print("###CONTATO3### coluna atraves do aro, pod baixado ao assoalho + saia, pescoco vertical")
print("###CONTATO2### pescoco vertical, saia do pod, mount em Y, mao no aro, braco, coluna, endplates, torso")
print("###PODS### SP_L/SP_R reconstruidos como SOLIDOS FECHADOS (cube->taper->bisect->bevel2)")
print("###CONTATO_FIX### pescoco solido, mao NO aro, braco->ombro, coluna assoalho->aro, endplates na barra, mount+saia do pod")
print("###FIXCONTATO### C_Col estendida, maos no aro, bracos ate as maos, endplates encostados, mounts ate o pod, longarina do motor")
print("###FLUT### 8 flutuantes -> 0 (todos deleted ou conectados)")
bpy.ops.wm.save_as_mainfile(filepath='/opt/blender-runner/outputs/B157.blend')
print('###B017### ok')
