...
ALVO = MD + "authored/conjunto-v163.blend"
...
sc.world.node_tree.nodes["Background"].inputs[0].default_value = (0.55, 0.55, 0.57, 1)  # FUNDO CLARO: comparavel ao concept (grid claro ~197 sRGB)
...
for tag, loc, rot, sinal in VIEWS:
    ...
    sc.render.filepath = "/tmp/R164_%s.png" % tag
    bpy.ops.render.render(write_still=True)
    print("###R42### render", tag)
for o in bpy.data.objects:
    o.hide_render = False
json.dump({"cx": cx, "cz": cz, "OS": ext * 1.30, "RES": 620, "md5": H_ALVO}, open("/tmp/r164_map.json", "w"))
