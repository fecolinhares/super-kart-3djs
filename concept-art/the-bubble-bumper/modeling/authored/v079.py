...
for tag, x, yc, r, w in (("FL", XF, +0.6390, 0.133, 0.163), ("FR", XF, -0.6390, 0.133, 0.163),
                         ("RL", XR, +0.5780, 0.165, 0.285), ("RR", XR, -0.5780, 0.165, 0.285)):
    anel3d("TIRE_" + tag, x, yc, r, r, r * 0.60, w, v=48, m=M_TIRE, rborda=(0.030 if "R" in tag else 0.018))
    _s = 1.0 if yc > 0 else -1.0
    cil("RIM_" + tag, x, yc + _s * w * 0.42, r, r * 0.62, w * 0.14, m=M_METAL, v=32, smooth=False)   # aro cinza (visivel no anel)
    cil("RING_" + tag, x, yc + _s * w * 0.46, r, r * 0.42, w * 0.08, m=M_ACC, v=28, smooth=False)     # anel amarelo
    cil("HUB_" + tag, x, yc + _s * w * 0.44, r, r * 0.17, w * 0.06, m=M_METAL, v=20, smooth=False)   # cubo
...
cil("R_ExhC", -0.72, 0.0, 0.330, 0.075, 0.46, rot=(0, math.radians(90), 0), m=M_CHROME, v=26)  # REGRA 251: tubo ate x=-0.95
# LATERAIS em V para fora (concept: inclinados para fora) + mais longos para lerem no REAR
cil("R_ExhL_1", -0.72, 0.265, 0.390, 0.055, 0.48, rot=(0, math.radians(90), math.radians(-12)), m=M_CHROME, v=18)  # v073: maior + mais fora
cil("R_ExhL_-1", -0.72, -0.265, 0.390, 0.055, 0.48, rot=(0, math.radians(90), math.radians(12)), m=M_CHROME, v=18)
# bocas escavadas (furo): anel preto na ponta de cada escape
cil("R_MouthC", -0.955, 0.0, 0.330, 0.050, 0.030, rot=(0, math.radians(90), 0), m=M_TIRE, v=24)
...
cil("R_BumperBar", -1.148, 0.0, 0.140, 0.030, 1.20, rot=(math.radians(90), 0, 0), m=M_BODY, v=14)  # REGRA 252: barra ABAIXO dos escapes — nunca ocluir a assinatura
...
box("Seat_Back", -0.30, -0.46, -0.145, 0.145, 0.14, 0.62, m=M_WHITE, bevel=0.022)  # reclinado ~40°
box("Seat_Base", -0.48, 0.05, -0.145, 0.145, 0.070, 0.55, m=M_WHITE, bevel=0.015)  # v062: sobe ao torso
...
for _k in range(3):
    ...
    for (_r,_sg) in ((_ra,+1),(_ra,-1),(_rb,-1),(_rb,+1)):
        ...
        _v.append(bmT.verts.new((_vx,_vy*_ca-0.004*_sa,0.648+_vy*_sa+0.004*_ca)))
    bmT.faces.new(_v)
...
bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v079.blend")
