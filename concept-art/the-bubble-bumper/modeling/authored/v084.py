...
for tag, x, yc, r, w in (("FL", XF, +0.6390, 0.133, 0.163), ("FR", XF, -0.6390, 0.133, 0.163),
    ...
    anel3d("TIRE_" + tag, x, yc, r + 0.002, r, r * 0.28, w, v=48, m=M_TIRE, rborda=(0.046 if "R" in tag else 0.030))   # v082d: furo 0.60r -> 0.28r (banda de PNEU grossa; o aro de 0.30r preenche o furo)
    _s = 1.0 if yc > 0 else -1.0
    cil("RIM_" + tag, x, yc + _s * w * 0.30, r + 0.002, r * 0.32, w * 0.14, m=M_METAL, v=32, smooth=False)   # v082: era r*0.62 (cobria 62% do pneu)   # aro cinza (visivel no anel)
    cil("RING_" + tag, x, yc + _s * w * 0.36, r + 0.002, r * 0.22, w * 0.08, m=M_ACC, v=28, smooth=False)   # v082: era r*0.42     # anel amarelo
    cil("HUB_" + tag, x, yc + _s * w * 0.42, r + 0.002, r * 0.08, w * 0.06, m=M_METAL, v=20, smooth=False)   # v082: era r*0.17   # cubo
...
for _tag, _x, _yc, _r, _w in (("FL", XF, +0.6390, 0.133, 0.163), ("FR", XF, -0.6390, 0.133, 0.163),
                              ("RL", XR, +0.5600, 0.1775, 0.320), ("RR", XR, -0.5600, 0.1775, 0.320)):
    for _k in (-1, 0, 1):
        anel3d("TRD_%s_%d" % (_tag, _k + 2), _x, _yc + _k * _w * 0.22, _r + 0.002, _r + 0.002, _r * 0.90,
              _w * 0.085, v=32, m=M_METAL)   # v082c: ANEL VAZADO — o cilindro cheio virava DISCO cinza cobrindo o pneu
...
def sec_U(yo, zb, zt, p=0.55):
    ...
    return pts
loft("N_Nose", [
    (1.175, sec_U(0.165, 0.030, 0.185)),  # v072: base desce p/ overlap real com a cinta   # v071: ponta ROMBA vertical (topo 0.185 ~ 0.15 da base)
    (1.145, sec_U(0.158, 0.032, 0.230)),
    (1.070, sec_U(0.148, 0.046, 0.258)),
    (0.960, sec_U(0.142, 0.043, 0.272)),
    (0.840, sec_U(0.148, 0.041, 0.279)),
    (0.780, sec_U(0.145, 0.040, 0.275)),
...
box("N_GrillBG", 1.160, 1.162, -0.150, 0.150, 0.06, 0.15, m=M_TIRE, bevel=0.004)  # v070: recesso na face do BLOCO (topo 0.15)
loft("F_Bow", [
...
for sgn in (+1, -1):
    box("F_ArmU_%d" % sgn, 0.630, 0.98, sgn * 0.130, sgn * 0.46, 0.035, 0.135, m=M_ACC, bevel=0.012)  # v072: EMBUTIDO na cinta (fim do trilho solto)
...
POD = [(0.45, 0.140, 0.230, 0.300), (0.34, 0.260, 0.180, 0.360), (0.18, 0.360, 0.150, 0.415),
...
# ---------- MOTOR + 3 ESCAPES ----------
box("R_Motor", -0.90, -0.60, -0.190, 0.190, 0.20, 0.50, m=M_METAL, bevel=0.030)  # v078: Y +/-0.19 -> toca a longarina
for _k in range(4):
...
cil("R_ExhC", -0.755, 0.0, 0.360, 0.086, 0.535, rot=(0, math.radians(90), 0), m=M_CHROME, v=26)  # v082: maior + protruso (x=-1.02) + agrupado/mais alto
# LATERAIS em V para fora (concept: inclinados para fora) + mais longos para lerem no REAR
cil("R_ExhL_1", -0.735, 0.275, 0.415, 0.066, 0.545, rot=(0, math.radians(90), math.radians(-12)), m=M_CHROME, v=18)  # v082: maior + protruso
cil("R_ExhL_-1", -0.735, -0.275, 0.415, 0.066, 0.545, rot=(0, math.radians(90), math.radians(12)), m=M_CHROME, v=18)
# bocas escavadas (furo): anel preto na ponta de cada escape
cil("R_MouthC", -1.034, 0.0, 0.360, 0.072, 0.030, rot=(0, math.radians(90), 0), m=M_TIRE, v=24)   # v082: boca central maior, na ponta
cil("R_RimC", -1.008, 0.0, 0.360, 0.088, 0.014, rot=(0, math.radians(90), 0), m=M_CHROME, v=26)  # v082: borda prateada da boca
for _s in (+1, -1):
...
for _k in range(5):
    _yy = -0.10 + 0.05 * _k
    box("R_Dif%d" % _k, -1.02, -0.80, _yy - 0.009, _yy + 0.009, 0.10, 0.26, m=M_BODY, bevel=0.004)
# v078 (AUDITORIA): conjunto traseiro (motor/escapes/difusor/barra) era ilha com 0.25 m de vao
for _s in (+1, -1):
    cil("Rail_R_%d" % _s, -0.820, _s * 0.205, 0.235, 0.050, 0.680, rot=(0, math.radians(90), 0), m=M_BODY, v=14, smooth=False)
...
box("R_Grille", -1.055, -1.005, -0.330, 0.330, 0.360, 0.560, m=M_BODY, bevel=0.010)   # v082: painel AZUL acima da barra
...
rings=[]
CORDA=0.26
for (x) in (-0.900, -0.960, -1.030, -1.160):
    t=(-0.900-x)/CORDA if x<-0.900 else 0.0     # 0 na borda de ataque (frente), 1 na saida
    ring=[]
    n=14
    for j in range(n):
        a=2*math.pi*j/n
        yy=0.475*math.cos(a)  # v078 AUDITORIA: concept 0.945-0.9685 m (66-67%W) por 2 metodos; 0.586 era ERRO
        # gota: espessura MAXIMA no bordo de ataque (t=0), afinando ate 40% na saida
        esp=0.105*(1.0-0.60*t)                   # 0.105 frente -> 0.042 tras
        zz=0.855-0.036*t-esp*0.5*(1-math.cos(a))  # sobe a secao: topo redondo
...
bmA.faces.new(list(reversed(rings[0]))); bmA.faces.new(list(rings[-1]))   # v078: era CASCA ABERTA (28 boundary edges)
o_A=ob(bmA,"R_Wing",M_BODY)  # v073: ASA AZUL (regra 254 — cor e assinatura)
for sgn in (+1, -1):
...
_oT = esf("P_Torso", tuple(TORSO + Vector((0, 0, 0.02))), _dT.length / 2 + 0.05, 0.155, 0.135, m=M_SUIT, seg=24)   # v066: barril ALINHADO ao tronco deitado
_oT.rotation_euler = _dTn.to_track_quat("X", "Z").to_euler()
esf("P_Waist", tuple(HIP + (SHO - HIP) * 0.75), 0.085, 0.145, 0.115, m=M_SUIT, seg=18)     # cintura fina
...
esf("P_Visor", tuple(HEA + Vector((0.098, 0, -0.010))), 0.070, 0.150, 0.095, m=M_VIS, seg=20)
...
esf("P_Shoulder", tuple(SHO + Vector((-0.02, 0, 0.01))), 0.145, 0.255, 0.085, m=M_ACC, seg=20)
...
box("P_FacePlate", -0.020, 0.048, -0.108, 0.108, 0.995, 1.142, m=M_VIS, bevel=0.010)
...
for _s in (+1, -1):
    esf("P_Eye_%d" % _s, (0.030, _s * 0.060, 1.118), 0.033, 0.033, 0.033, m=M_WHITE, seg=18)
    esf("P_Pup_%d" % _s, (0.050, _s * 0.060, 1.118), 0.018, 0.018, 0.018, m=M_TIRE, seg=14)
...
bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v084.blend")
