...
loft("N_Nose", [
 ...
 ], M_ACC)  # v107: NARIZ AMARELO (concept: frente 100% amarela, 0 azul) # v071: FRENTE VERTICAL ROMBA - capsula curta, nao cone horizontal
...
loft("F_Bow", [
    (1.175, sec_U(0.350, 0.025, 0.190, p=0.45)),
    (1.155, sec_U(0.345, 0.025, 0.191, p=0.45)),
    (1.120, sec_U(0.330, 0.027, 0.192, p=0.45)),
    (1.060, sec_U(0.270, 0.035, 0.180, p=0.45)),
    (1.000, sec_U(0.280, 0.038, 0.182, p=0.45)),
    (0.980, sec_U(0.270, 0.040, 0.184, p=0.45)),
...
for sgn in (+1, -1):
    box("F_Pad_%d" % sgn, 0.680, 1.160, sgn * 0.150, sgn * 0.580, 0.030, 0.245, m=M_ACC, bevel=0.030)  # v109  # v074: SECAO larga (vision lia esfera como FAROL)
...
for sgn in (+1, -1):
    cil("ChassisF_%d" % sgn, 0.810, sgn * 0.205, 0.235, 0.055, 0.62, rot=(0, math.radians(90), 0), m=M_ACC, v=14, smooth=False)   # v107b: FRENTE amarela (x 0.50..1.12)
    cil("Chassis_%d" % sgn, 0.000, sgn * 0.205, 0.235, 0.055, 1.02, rot=(0, math.radians(90), 0), m=M_BODY, v=14, smooth=False)   # v107b: TRAS azul (x -0.51..0.51) — sobrepoe p/ nao criar vao
...
for _s3 in (+1, -1):
    esf("R_MCowl_%d" % _s3, (-0.745, _s3 * 0.190, 0.360), 0.205, 0.078, 0.180, m=M_BODY, seg=22)   # lateral da carenagem
esf("R_MCowlTop", (-0.760, 0.0, 0.495), 0.185, 0.205, 0.075, m=M_BODY, seg=22)                      # tampa superior
esf("R_MCowlBack", (-0.560, 0.0, 0.360), 0.072, 0.198, 0.170, m=M_BODY, seg=20)                    # traseira da carenagem
...
for _s in (+1, -1):   # v082: U-bend — o quadro cinza curva para cima nas duas pontas (concept)
    cil("R_UBend_%d" % _s, -1.139, _s * 0.585, 0.275, 0.024, 0.350, rot=(0, 0, 0), v=14, m=M_METAL)
cil("R_UBendTop", -1.139, 0.0, 0.450, 0.022, 1.170, rot=(math.radians(90), 0, 0), m=M_ACC, v=14)   # v083b: barra SUPERIOR fecha o U (concept)   # v082b: VERTICAL (rot default X90 punha o eixo em Y) e cruzando a barra
...
bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v111.blend")
