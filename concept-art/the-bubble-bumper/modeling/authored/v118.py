...
rings=[]
CORDA=0.26
# v118: 8 estacoes (era 4) + CAMBER no topo (era linear) -> a asa le como peca CURVA, nao retangular
EST=[-0.900,-0.935,-0.970,-1.005,-1.040,-1.080,-1.120,-1.160]
for (x) in EST:
    ...
    for j in range(n):
        a=2*math.pi*j/n
        yy=0.475*math.cos(a)
        esp=0.105*(1.0-0.60*t)
        # topo em ARCO (camber): desce rapido no bordo de ataque e achata na saida (curva convexa)
        ztop=0.855-0.012*t-0.024*t*t
        zz=ztop-esp*0.5*(1-math.cos(a))
...
bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v118.blend")
