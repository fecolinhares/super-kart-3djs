...
if _hm:
    ...
    # v142: SUBDIVIDIR as faces que cruzam a zona, senao faces grandes nunca passam no teste
    import bmesh as _bmmod
    _zb = (0.960, 1.100, -0.307, -0.137)
    _bm = _bmmod.new(); _bm.from_mesh(_hm.data); _bm.faces.ensure_lookup_table()
    _eds = set()
    for _f in _bm.faces:
        for _v in _f.verts:
            _q = _mw @ _v.co
            if _zb[2] <= _q.x <= _zb[3] and _zb[0] <= _q.z <= _zb[1]:
                for _e in _f.edges:
                    _eds.add(_e)
                break
    if _eds:
        _bmmod.ops.subdivide_edges(_bm, edges=list(_eds), cuts=2)
    _bm.to_mesh(_hm.data); _bm.free()
    print('###V142### arestas subdivididas na zona: %d' % len(_eds))
    _k = 0
...
for _nm in _EYES:
    ...
    if _o2:
        _o2.location.z += _DZ_EYE; _n5 += 1
print('###V140### olhos/visor subidos %.3f m em %d objetos' % (_DZ_EYE, _n5))
bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v142.blend")
