...
esf("P_Shoulder", tuple(SHO + Vector((0.005, 0, 0.0555))), 0.170, 0.222, 0.1195, m=M_SUIT, seg=20)  # v145: topo 1.039 -> 1.000 e FUNDO PRESERVADO 0.761 (centro -0.0195 E rz -0.0195). CAUSA MEDIDA (regra 318): P_Shoulder |y|max 0.215 > casco 0.167 -> na vista lateral ele fica NA FRENTE do casco e a superficie visivel em z 1.00..1.04 era o OMBRO azul, nao o casco. Concept: z=0.96 = azul, z=1.02 = claro -> transicao em ~0.98..1.00.  # v134: M_ACC->M_SUIT. MEDIDO na grade de cores: o concept e AZUL em x -0.38..-0.14 / z 1.05..0.90 (todo o tronco/ombro) e o modelo era AMARELO ali. O amarelo do concept e friso (so em x=-0.14 z=1.05 e x=-0.38 z=1.00).  # v129: centro z +0.065 e rz 0.082->0.140 -> ombro z 0.77..1.05 = MEDIDO no concept (blob azul 0.86<z<1.05 com x -0.480..-0.137); antes topava em 0.916 e deixava 129 mm de vao sob a frente do capacete  # v128: rx 0.140->0.170 e centro +0.025 -> tronco x -0.475..-0.135 = MEDIDO no concept (-0.480..-0.137); antes terminava em -0.195 e o capacete (-0.147) ficava flutuando 0.05 m a frente222
...
if _hm:
    ...
    for _p in _hm.data.polygons:
        _c = _mw @ _p.center
        _vs = [_mw @ _hm.data.vertices[_vi].co for _vi in _p.vertices]
        if all(-0.307 <= _q.x <= -0.137 and 0.960 <= _q.z <= 1.100 for _q in _vs):
...
bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v145.blend")
