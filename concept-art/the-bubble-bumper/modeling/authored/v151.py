...
M_VISL = mat("Visor_Grey", (0.30, 0.33, 0.37), 0.35)
M_VILL = mat("Visor_Light", (0.62, 0.66, 0.73), 0.28)  # v137: medido no concept RGB ~(130-200)   # v086: visor cinza perceptivel
...
bmA=bmesh.new()
rings=[]
CORDA=0.192   # v122: MEDIDO no concept (era 0.26) - blob amarelo em side.jpg: 81px/992px de comprimento
XFF=-0.700    # v122: borda de ataque medida no concept (era -0.900, isto e, 0.20 m mais para tras)
# concept: x_world -0.889..-0.700 | z 0.731..0.810 (topo) -> corda 0.192, espessura max 0.079 (era 0.105)
EST=[-0.700,-0.724,-0.748,-0.772,-0.796,-0.820,-0.844,-0.868,-0.892]
for (x) in EST:
    t=(XFF-x)/CORDA
    t=min(max(t,0.0),1.0)
    ring=[]
    ...
    for j in range(n):
        a=2*math.pi*j/n
        yy=0.475*math.cos(a)
        esp=0.079*math.sqrt(max(1.0-(2*t-1)**2,0.0))
        ztop=0.812-0.014*t+0.006*math.sqrt(max(1.0-(2*t-1)**2,0.0))
        # v124: ELIPSE REAL. Antes era zz=ztop-esp*0.5*(1-cos(a)) = LINEAR em cos(a):
        # a secao degenerava (area ~0, a=pi/2 e a=3pi/2 caiam no mesmo ponto) -> a asa era uma
        # CHAPA SEM VOLUME. Foi por isso que o vision disse 'flat/retangular' em todos os ciclos
        # e porque nenhum ajuste de PERFIL resolvia: eu calibrava contorno sobre superficie sem espessura.
        zc=ztop-esp*0.5
        zz=zc+esp*0.5*math.sin(a)
...
for sgn in (+1, -1):
    box("R_Endp_%d" % sgn, -1.165, -0.890, sgn * 0.453, sgn * 0.537, 0.650, 0.810, m=M_ACC, bevel=0.006)  # v135: y 0.478->0.537 e z 0.70..0.93 -> 0.65..0.81. MEDIDO no perfil de largura FRONT: concept e largo (meia-largura 0.537) em z 0.65..0.80 e estreito (0.167-0.190) em z 0.85..0.90; o modelo tinha 0.269 em z=0.65 (deficit 0.27) e 0.476 em z 0.85..0.90 (excesso 0.29) porque os endplates subiam ate 0.93. Mesmo objeto, dois desvios opostos, uma correcao.  # v078: pontas da asa 0.95
    box("R_Pylon_%d" % sgn, -0.880, -0.770, sgn * 0.140, sgn * 0.270, 0.19, 0.78, m=M_METAL, bevel=0.010)  # v123: pylons seguem a asa (-0.700..-0.892)  # v078: desce ate a longarina (era ilha)
...
_oN = esf("P_Neck", (-0.23, 0.0, 0.930), 0.105, 0.115, 0.125, m=M_SUIT, seg=14)  # v131: rz 0.098->0.125 -> topo do pescoco 1.055 ATRAVESSA o ombro (topo 1.039). Razao: P_Shoulder e superficie FECHADA e BVH.overlap mede INTERSECCAO, nao contencao: com o pescoco inteiro dentro do ombro o gate acusa 0 pares.  # v130: rz 0.072->0.098 e centro z 0.945->0.930 -> garante interseccao real com o ombro elevado (v129 quebrou o contato Shoulder<->Neck)  # v126: era (0.090,0.075,0.055) em z 0.92 -> vao de 2.2 cm entre capacete(z0.938) e ombro(z0.916) lia como cabeca DESLIGADA; agora z 0.873..1.017 atravessa os dois
esf("P_Helmet", tuple(HEA + Vector((-0.01, 0, 0.012))), 0.163, 0.170, 0.1575, m=M_HELM, seg=28)  # topo=1.2523
esf("P_Visor", tuple(HEA + Vector((0.086, 0, 0.014))), 0.030, 0.156, 0.030, m=M_VISL, seg=20)  # v141: fenda fina (era massa 0.078/0.082)   # v086: cinza + acima dos olhos
...
esf("P_Shoulder", tuple(SHO + Vector((0.005, 0, 0.0555))), 0.170, 0.222, 0.1195, m=M_SUIT, seg=20)  # v146: topo 1.039->1.000, fundo preservado (regra 318: ombro |y|0.215 ocluia o casco 0.167)  # v134: M_ACC->M_SUIT. MEDIDO na grade de cores: o concept e AZUL em x -0.38..-0.14 / z 1.05..0.90 (todo o tronco/ombro) e o modelo era AMARELO ali. O amarelo do concept e friso (so em x=-0.14 z=1.05 e x=-0.38 z=1.00).  # v129: centro z +0.065 e rz 0.082->0.140 -> ombro z 0.77..1.05 = MEDIDO no concept (blob azul 0.86<z<1.05 com x -0.480..-0.137); antes topava em 0.916 e deixava 129 mm de vao sob a frente do capacete  # v128: rx 0.140->0.170 e centro +0.025 -> tronco x -0.475..-0.135 = MEDIDO no concept (-0.480..-0.137); antes terminava em -0.195 e o capacete (-0.147) ficava flutuando 0.05 m a frente222
...
box("P_FacePlate", -0.160, 0.010, -0.108, 0.108, 0.960, 1.100, m=M_VILL, bevel=0.010)  # v138: MEDIDO concept x -0.30..-0.14 (0.16 m), z 0.96..1.10  # v137: claro/cromado   # v085: recuada (antes cobria os olhos)
...
# === v125: MEDICAO por faixa de z (regra 305) ===
# concept (side.jpg segmentado, KX=0.003117 KZ=0.003146): blob azul z>1.05 = CAPACETE em
#   x -0.480..-0.134 (centro -0.307, profundidade 0.346 m); ombros z 0.86..1.05 = x -0.480..-0.137.
# modelo v124: P_Helmet centro -0.160 | P_Shoulder centro -0.330 -> a CABECA esta 0.147 m a FRENTE
#   do tronco. No concept o capacete fica praticamente EM CIMA do tronco (centros -0.307 vs -0.330).
# Acao: translacao RIGIDA da cabeca para tras (mantem o conjunto coerente) + pescoco a 45%.
# === v127: ENCOSTO DO BANCO ===
# Medicao coluna-a-coluna em z 0.698..1.099 mostrou vao de 134 mm em x=-0.450 entre o capacete
# (base 1.010 ali, porque o elipsoide sobe na traseira) e o ombro (topo 0.876). Nas colunas
# x=-0.35/-0.31 NAO ha vao (e por onde passa o pescoco) -> o vao e' TRASEIRO e nenhuma peca
# existente alcanca. No concept esse vazio e' preenchido pelo ENCOSTO DO BANCO (escuro, atras
# das costas) -> era o que fazia a cabeca ler como DESLIGADA do corpo.
box("P_SeatBack", -0.520, -0.380, -0.150, 0.150, 0.640, 0.920, m=M_VISL, bevel=0.030)  # v133: topo 1.045->0.920 e frente -0.290->-0.380. MEDIDO: no concept em x=-0.33 a cor de z=0.87 a 1.05 e AZUL continuo (piloto) e o encosto so aparece em z<=0.84; no v132 o encosto subia a 1.045 e ocultava o pescoco, pintando uma faixa CINZA entre o capacete azul e o ombro amarelo - o vision lia isso como VAO. Silhueta continua nao garante leitura continua: a COR visivel na vista decide.  # v132: -0.400->-0.290 e topo 1.030->1.045. Razao: medicao coluna-a-coluna mostrou o concept CONTINUO em x=-0.33 (0.701..1.089) e x=-0.28 (0.701..1.107), enquanto o modelo tinha vaos; no concept o encosto e uma massa alta e continua atras/lateral do piloto, nao uma chapa atras dele.
print('###V127### encosto do banco adicionado: x -0.520..-0.400 z 0.600..1.030')
_DX_HEA = -0.147
_HEADS = ('P_Helmet','P_Visor','P_HCrest','P_HSpoil','P_HVent','P_Stripe','P_StripeF',
          'P_Eye_1','P_Eye_-1','P_Pup_1','P_Pup_-1','P_Mouth_1','P_Mouth_-1','P_Mouth_M','P_FacePlate')
_n=0
for _o in bpy.data.objects:
    if _o.name in _HEADS:
        _o.location.x += _DX_HEA; _n+=1
print('###V125### cabeca transladada %.3f m para tras em %d objetos'%(_DX_HEA,_n))
_o=bpy.data.objects.get('P_Neck')
if _o: _o.location.x += _DX_HEA*0.45; print('###V125### pescoco deslocado %.3f m'%(_DX_HEA*0.45))
# v139: REATRIBUIR MATERIAL das faces do casco na zona medida do painel claro
# (REGRA: nunca apagar faces de superficie fechada -> abre buraco/non-manifold)
_hm = bpy.data.objects.get('P_Helmet')
if _hm:
    _names = [ms.material.name for ms in _hm.material_slots if ms.material]
    if M_VILL.name not in _names:
        _hm.data.materials.append(M_VILL); _names = [ms.material.name for ms in _hm.material_slots if ms.material]
    _idx = _names.index(M_VILL.name)
    # v147: ATUALIZAR o depsgraph ANTES de ler matrix_world. O bloco _HEADS muda location.x do
    # P_Helmet no fim do script e matrix_world fica OBSOLETO -> a zona era aplicada com coordenadas
    # defasadas em 0.147 m (virava x -0.454..-0.284 em mundo). Provado por raycast: faces que
    # satisfaziam inX=inZ=inNY=1 continuavam Helmet_Blue.
    bpy.context.view_layer.update()
    _mw = _hm.matrix_world
    _k = 0
    for _p in _hm.data.polygons:
        _c = _mw @ _p.center
        _c = _mw @ _p.center
        _n = (_mw.to_3x3() @ _p.normal)
        _nl = _n.length
        if _nl > 1e-9:
            _n = _n / _nl
        # v144: pintar a SUPERFICIE VISIVEL na vista lateral = faces com normal apontando para a
        # camera (|ny| alto). O teste anterior pintava faces que nao sao a superficie visivel.
        # v151: borda INFERIOR MEDIDA no concept (nao mais o limite fixo 0.960): a faixa
        # clara termina em diagonal. Borda do concept por coluna: x=-0.30 -> z=1.040 |
        # -0.26 -> 0.980 | -0.22 -> 0.940 | -0.18 -> 0.920 | -0.15 -> 0.915 | -0.12 -> 0.900.
        # Ajuste linear: z_min(x) = 0.924 - 0.778*(x+0.15). O limite fixo pintava BAIXO DEMAIS
        # atras (-0.025 em x=-0.30) e PARAVA CEDO na frente (+0.040 em x=-0.18).
        if (-0.307 <= _c.x <= -0.137 and _c.z >= 0.924 - 0.778 * (_c.x + 0.15)
                and _c.z <= 1.100 and abs(_n.y) > 0.60):
            _p.material_index = _idx; _k += 1
    print('###V139### faces do casco repintadas M_VILL: %d de %d' % (_k, len(_hm.data.polygons)))
# v140: subir olhos/pupilas/visor 0.05 m (concept tem a fenda escura em z 1.06..1.08)
# v143: REVERTER a subida de 0.05 dos olhos (sem ganho medido e quebrou o contato
# P_Eye_1<->P_FacePlate desde o v140) e BAIXAR so o visor para a fenda medida (z 1.06..1.08)
_n6 = 0
for _nm in ('P_Eye_1','P_Eye_-1','P_Pup_1','P_Pup_-1'):
    _o2 = bpy.data.objects.get(_nm)
    if _o2:
        _o2.location.z += -0.050; _n6 += 1
_o2 = bpy.data.objects.get('P_Visor')
if _o2:
    _o2.location.z += -0.080; _n6 += 1
print('###V143### revertidos/ajustados em z: %d objetos' % _n6)
# v148: P_FacePlate OCUPA O PIXEL e estava SEM MATERIAL -> renderizava com o cinza padrao do
# Blender, que o classificador lia como FUNDO ("."). Provado por raycast v147: em x=-0.18 e
# x=-0.14, z 0.96..1.09 o hit e P_FacePlate com material nao resolvido ('-'). O concept quer
# CLARO nessa regiao (x -0.300..-0.140, z 0.96..1.10, RGB ~203,203,205). Nao e vao de geometria.
_pfp = bpy.data.objects.get('P_FacePlate')
if _pfp is not None and _pfp.type == 'MESH':
    bpy.context.view_layer.update()
    _mv = bpy.data.materials.get('Visor_Light')
    assert _mv is not None, 'material Visor_Light inexistente'
    _nomes = [m.name if m else None for m in _pfp.data.materials]
    if 'Visor_Light' not in _nomes:
        _pfp.data.materials.append(_mv)
        _nomes = [m.name if m else None for m in _pfp.data.materials]
    _idx = _nomes.index('Visor_Light')
    _nf = 0
    for _p in _pfp.data.polygons:
        _p.material_index = _idx
        _nf += 1
    print('###V148### P_FacePlate: %d faces -> Visor_Light (idx %d) | slots=%s' % (_nf, _idx, _nomes))
    # v149: o RENDER nao mudou com o material na DATA (v148 deu exatamente os mesmos 63% do v147,
    # pixel a pixel). Causa: o slot de nivel OBJETO (link='OBJECT') esta nulo e e ele que o
    # renderizador usa. Corrigir no nivel do objeto tambem.
    _links = []
    for _sl in _pfp.material_slots:
        _links.append(_sl.link)
        if _sl.link == 'OBJECT' and _sl.material is None:
            _sl.material = _mv
    _conf = [(sl.link, sl.material.name if sl.material else None) for sl in _pfp.material_slots]
    print('###V149### slots do OBJETO: %s' % _conf)
else:
    print('###V148### ERRO: P_FacePlate nao encontrado')

# v150: OLHOS/PUPILAS FORA DA CABECA. Medido: P_Helmet x -0.467..-0.147 | P_FacePlate -0.307..-0.137
# | P_Eye_1/-1 x -0.139..-0.065 | P_Pup x -0.095..-0.055. Ou seja os olhos estao ATRAS do casco,
# fora da cabeca, encostados na borda traseira do face plate (contato que o gate reporta como
# valido). O raio confirma: em z 1.04..1.10 a superficie mais a frente e P_Eye_1/P_Pup_1.
# O concept mostra um VISOR CLARO OPACO (RGB ~203,203,205) -> os olhos devem ficar DENTRO do casco,
# na zona do visor (x -0.28..-0.16), sem aparecer na silhueta lateral.
# Recolher em X (nao em Z: a subida em z quebrou contato no v140/v141).
_DX_OLHO = -0.10
bpy.context.view_layer.update()
for _nm in ('P_Eye_1', 'P_Eye_-1', 'P_Pup_1', 'P_Pup_-1'):
    _ob = bpy.data.objects.get(_nm)
    if _ob is None:
        print('###V150### ERRO: %s nao existe' % _nm)
        continue
    _pts = [_ob.matrix_world @ Vector(c) for c in _ob.bound_box]
    _x0 = min(p.x for p in _pts)
    if _ob.parent is None:
        _ob.location.x += _DX_OLHO
    else:
        _ob.matrix_world = Matrix.Translation((_DX_OLHO, 0, 0)) @ _ob.matrix_world
    bpy.context.view_layer.update()
    _pts2 = [_ob.matrix_world @ Vector(c) for c in _ob.bound_box]
    _x1 = min(p.x for p in _pts2)
    print('###V150### %-9s x_min %.3f -> %.3f (parent=%s)' % (_nm, _x0, _x1, _ob.parent.name if _ob.parent else 'None'))

bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v151.blend")
