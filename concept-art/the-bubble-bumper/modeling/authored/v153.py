...
bpy.ops.wm.open_mainfile(filepath=MD + "authored/conjunto-v152.blend")
...
print("###V153### P_Shoulder antes: x %.3f..%.3f | z %.3f..%.3f" % (x0, x1, z0, z1))
...
print("###V153### t: inicio=%.3f | x=-0.18 -> %.3f | x=-0.16 -> %.3f | fim=%.3f" % (t_ini, t_035, t_017, t_fim))
...
zspan = z1 - z0
# v153: o peso por altura GLOBAL so dava 0.63 no vertice mais alto da coluna (o topo da coluna
# fica em h=0.81 do bbox), entao 0.035 virava 0.020. Peso agora e a altura LOCAL da coluna
# (topo da coluna = 1.0) elevada ao quadrado -> no topo a queda e integral e a base fica intacta.
BINS = 80
topo_bin = {}
for v in me.vertices:
    w = mw @ v.co
    b = int(max(0.0, min(1.0, (w.x - x0) / (x1 - x0))) * BINS)
    if w.z > topo_bin.get(b, -1e9):
        topo_bin[b] = w.z
afetados = 0
for v in me.vertices:
    w = mw @ v.co
    ...
    if d <= 0:
        continue
    b = int(max(0.0, min(1.0, t)) * BINS)
    ztopo_col = topo_bin.get(b, z1)
    hloc = (w.z - z0) / (ztopo_col - z0) if ztopo_col > z0 else 1.0
    peso = max(0.0, min(1.0, hloc)) ** 2
    if peso <= 0:
        continue
    w.z -= d * peso
    v.co = inv @ w
...
me.update()
bpy.context.view_layer.update()
print("###V153### vertices deformados: %d de %d" % (afetados, len(me.vertices)))

pts2 = [ob.matrix_world @ Vector(c) for c in ob.bound_box]
print("###V153### P_Shoulder depois: x %.3f..%.3f | z %.3f..%.3f" % (
...
DIR = Vector((0, -1, 0))
print("###V153### topo do ombro DEPOIS (alvo entre parenteses):")
for x, alvo in ((-0.22, 0.940), (-0.20, 0.930), (-0.18, 0.920), (-0.16, 0.918), (-0.15, 0.915)):
    ...
    while z >= 0.80:
        ...
        if ok and obj.name == 'P_Shoulder':
            ...
            break
        z = round(z - 0.005, 4)
    print("###V153###   x=%+.2f -> topo=%s (alvo %.3f)" % (x, ('%.3f' % topo) if topo else 'ausente', alvo))

assert 'conjunto-v152.blend' not in str(ob), 'sanidade'
bpy.ops.wm.save_as_mainfile(filepath=MD + "authored/conjunto-v152.blend")
print("###V153### salvo conjunto-v152.blend")
