...
SRC = MD + "authored/conjunto-v154.blend"
DST = MD + "authored/conjunto-v155.blend"
...
print("###V155### P_FacePlate antes: x %.3f..%.3f | z %.3f..%.3f | verts=%d" % (x0, x1, z0, z1, len(me.vertices)))

NOVO_FUNDO = 0.918
delta = NOVO_FUNDO - z0
assert abs(delta) < 0.05, 'delta fora de faixa (+-0.05)'
moveram = 0
for v in me.vertices:
    w = mw @ v.co
    if abs(w.z - z0) < 0.008:      # so o fundo da placa
        w.z += delta
        v.co = inv @ w
        moveram += 1
me.update()
bpy.context.view_layer.update()
print("###V155### verts do fundo movidos: %d | delta %.3f" % (moveram, delta))

pts2 = [ob.matrix_world @ Vector(c) for c in ob.bound_box]
z0b = min(p.z for p in pts2); z1b = max(p.z for p in pts2)
print("###V155### P_FacePlate depois: z %.3f..%.3f (alvo fundo %.3f)" % (z0b, z1b, NOVO_FUNDO))
...
def mat_de(o, idx):
    try:
        ...
        pass
    return '-'
print("###V155### raio depois (x, z) -> objeto[material]:")
for x in (-0.20, -0.18, -0.16, -0.15):
    for z in (0.92, 0.94, 0.96):
        ok, loc, nor, idx, obj, m = sc.ray_cast(dg, Vector((x, 2.0, z)), DIR)
        if ok:
            print("###V155###   x=%+.2f z=%.2f -> %-13s [%s]" % (x, z, obj.name, mat_de(obj, idx)))
        else:
            print("###V155###   x=%+.2f z=%.2f -> VAZIO" % (x, z))

bpy.ops.wm.save_as_mainfile(filepath=DST)
print("###V155### salvo %s" % DST.split('/')[-1])
