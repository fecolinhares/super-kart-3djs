...
SRC = MD + "authored/conjunto-v161.blend"
DST = MD + "authored/conjunto-v162.blend"
...
LINHA = (-0.168, -0.140, 1.052, 1.078)   # x0, x1, z0, z1 da linha preta
...
def setbase(nome, rgb, rough=None):
    ...
    print("###V162### %s base %s -> %s%s" % (nome, ant, tuple(round(v, 3) for v in rgb),
...
print("###V162### M_BlackLine criado/ok")
...
for p in placa.data.polygons:
    ...
    if LINHA[0] <= c.x <= LINHA[1] and LINHA[2] <= c.z <= LINHA[3] and abs(nw.y) > 0.55:
        p.material_index = si
        pint += 1
print("###V162### faces da placa pintadas de PRETO (linha): %d" % pint)
assert pint >= 2, "linha preta sem faces (%d)" % pint

# ---- 2) OMRO: descer o topo em x=-0.22 em 0.010 (causa nomeada da celula (0.94,-0.22)) ----
bpy.context.view_layer.update()
om = bpy.data.objects.get("P_Shoulder")
assert om is not None, "P_Shoulder ausente"
mwo = om.matrix_world
alvo_x, queda = -0.22, 0.010
mov = 0
for v in om.data.vertices:
    w = mwo @ v.co
    if abs(w.x - alvo_x) <= 0.025 and w.z > 0.930:
        w.z -= queda
        v.co = mwo.inverted() @ w
        mov += 1
print("###V162### ombro: verts descidos em x~%.2f: %d (queda %.3f)" % (alvo_x, mov, queda))
assert mov >= 1, "nenhum vertice do ombro movido"
om.data.update()
bpy.context.view_layer.update()
bpy.ops.wm.save_as_mainfile(filepath=DST)
print("###V162### salvo:", DST)
