...
OUT=os.path.join(ROOT,'bubble-bumper-v017.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        ...
        box('POD_ACCENT_PANEL_L' if s<0 else 'POD_ACCENT_PANEL_R',(s*.455,.10,.535),(.018,.12,.020),MATS['accent_yellow'],.008)
    ...
    bumper_pts=[(-.62,.15,.22),(-.82,.30,.22),(-.86,.62,.22),(-.80,.94,.22),(-.62,1.12,.22),(0,1.16,.22),(.62,1.12,.22),(.80,.94,.22),(.86,.62,.22),(.82,.30,.22),(.62,.15,.22)]
    tube('BUMPER_U',bumper_pts,.06,MATS['body_blue'])
    sphere('U_HARDPOINT_FRONT_L',(-.62,.15,.22),.018,MATS['dark'])
    sphere('U_HARDPOINT_FRONT_R',(.62,.15,.22),.018,MATS['dark'])
    sphere('U_HARDPOINT_REAR_L',(-.80,.94,.22),.018,MATS['dark'])
    sphere('U_HARDPOINT_REAR_R',(.80,.94,.22),.018,MATS['dark'])
    ...
    # Explicit seated pilot chain: seat -> pelvis -> torso -> neck -> helmet; arms reach the wheel.
    ellipsoid('PILOT_PELVIS',(0,.08,.53),(.13,.14,.065),MATS['pilot'],16,10)
    ellipsoid('PILOT_TORSO',(0,.10,.64),(.11,.12,.14),MATS['pilot'],16,10)
    tube('PILOT_NECK',[(0,.10,.70),(0,.10,.76)],.045,MATS['pilot'])
    ellipsoid('PILOT_HELMET',(0,.11,.82),(.095,.10,.10),MATS['pilot'],18,10)
    ellipsoid('PILOT_VISOR',(0,.02,.83),(.068,.014,.032),MATS['glass'],16,8)
    for s in (-1,1):
        tube('PILOT_ARM_L' if s<0 else 'PILOT_ARM_R',[(s*.085,.08,.68),(s*.14,.42,.64),(s*.10,.68,.62)],.022,MATS['pilot_yellow'])
        tube('PILOT_THIGH_L' if s<0 else 'PILOT_THIGH_R',[(s*.08,.06,.53),(s*.11,-.10,.47),(s*.10,-.24,.40)],.032,MATS['pilot'])
    # Compact rear module: one central housing/grade plus exactly two visible side exhausts.
    housing=loft_y('REAR_HOUSING',[(-.48,0,.55,.16,.10),(-.68,0,.56,.19,.12),(-.82,0,.55,.16,.10)],MATS['metal'],18)
    housing['role']='rear_housing_integrated'
    torus('REAR_GRILLE',(0,-.86,.55),.095,.022,MATS['dark'],rot=(math.pi/2,0,0))
    for gx in (-.055,0,.055):
        box('REAR_GRILLE_SLOT_'+str(gx),(gx,-.885,.55),(.012,.012,.055),MATS['metal_dark'],.004)
    for s in (-1,1):
        tube('EXHAUST_L' if s<0 else 'EXHAUST_R',[(s*.25,-.72,.58),(s*.25,-.96,.58)],.045,MATS['metal'])
        sphere('EXHAUST_MOUNT_L' if s<0 else 'EXHAUST_MOUNT_R',(s*.25,-.72,.58),.018,MATS['metal_dark'])
        tube('REAR_SUSP_ARM_L' if s<0 else 'REAR_SUSP_ARM_R',[(s*.42,-.58,.43),(s*.53,-.64,.34),(s*.63,-.62,.30)],.022,MATS['metal_dark'])
        sphere('REAR_CHASSIS_MOUNT_L' if s<0 else 'REAR_CHASSIS_MOUNT_R',(s*.42,-.58,.43),.020,MATS['metal_dark'])
        sphere('REAR_UPRIGHT_MOUNT_L' if s<0 else 'REAR_UPRIGHT_MOUNT_R',(s*.63,-.62,.30),.020,MATS['metal_dark'])
    tube('REAR_LOW_BAR',[(-.48,-.90,.37),(0,-.94,.37),(.48,-.90,.37)],.028,MATS['metal_dark'])
    # presentation floor
...
def lights_and_camera():
    ...
    cams={
      ...
      'profile':((-4.0,.05,1.0),(0,.16,.52),2.60),
      'front':((0,4.0,1.0),(0,.48,.42),2.35),
      'rear':((0,-4.0,1.0),(0,-.42,.55),2.20),
