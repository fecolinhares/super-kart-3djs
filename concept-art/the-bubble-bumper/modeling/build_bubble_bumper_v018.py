...
OUT=os.path.join(ROOT,'bubble-bumper-v018.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    MATS['body_blue']=mat('Body Blue',(0.025,0.18,0.72),0.12,.30)
    MATS['accent_yellow']=mat('Custom Yellow',(0.98,0.72,0.02),0.04,.28)
    ...
    # Short front U bumper, only around the nose; not a loop around the whole kart.
    bumper=[(-.42,1.10,.23),(-.66,1.14,.23),(-.78,1.28,.23),(-.76,1.46,.23),(-.58,1.57,.23),(0,1.61,.23),(.58,1.57,.23),(.76,1.46,.23),(.78,1.28,.23),(.66,1.14,.23),(.42,1.10,.23)]
    tube('FRONT_BUMPER_U',bumper,.055,MATS['body_blue'])
    for s in (-1,1):
        tube('BUMPER_YELLOW_BAND_L' if s<0 else 'BUMPER_YELLOW_BAND_R',[(s*.72,1.25,.23),(s*.77,1.39,.23)],.058,MATS['accent_yellow'])
        sphere('BUMPER_SHELL_MOUNT_L' if s<0 else 'BUMPER_SHELL_MOUNT_R',(s*.42,1.10,.23),.018,MATS['metal_dark'])

    # Large concept side pods: yellow outer pod with a blue inset, connected to chassis.
    for s in (-1,1):
        ellipsoid('SIDE_POD_YELLOW_L' if s<0 else 'SIDE_POD_YELLOW_R',(s*.48,.00,.46),(.25,.62,.20),MATS['accent_yellow'],24,14)
        ellipsoid('SIDE_POD_BLUE_INSET_L' if s<0 else 'SIDE_POD_BLUE_INSET_R',(s*.48,.02,.52),(.19,.45,.11),MATS['body_blue'],24,12)
        tube('POD_CHASSIS_MOUNT_FRONT_L' if s<0 else 'POD_CHASSIS_MOUNT_FRONT_R',[(s*.28,.53,.43),(s*.42,.53,.44)],.025,MATS['metal_dark'])
        tube('POD_CHASSIS_MOUNT_REAR_L' if s<0 else 'POD_CHASSIS_MOUNT_REAR_R',[(s*.28,-.40,.43),(s*.46,-.40,.46)],.025,MATS['metal_dark'])
    ...
    ellipsoid('PILOT_HELMET',(0,.10,.98),(.16,.15,.16),MATS['body_blue'],24,16)
    ellipsoid('PILOT_VISOR',(0,-.04,.99),(.115,.025,.065),MATS['accent_yellow'],20,12)
    # Yellow stripe over helmet.
    box('HELMET_YELLOW_STRIPE',(0,.10,1.115),(.035,.13,.018),MATS['accent_yellow'],.008)
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='BLOCKOUT_018_CONCEPT_PROPORTIONS'; sc['D_tire_m']=.5
