...
OUT=os.path.join(ROOT,'bubble-bumper-v023.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    MATS['metal']=mat('Technical Silver',(0.30,0.34,0.40),0.65,.28)
    ...
    for s in (-1,1):
        box('SIDE_MIRROR_L' if s<0 else 'SIDE_MIRROR_R',(s*.68,-.42,.78),(.07,.09,.045),MATS['accent_yellow'],.018)
        tube('POD_PANEL_SEAM_L' if s<0 else 'POD_PANEL_SEAM_R',[(s*.59,-.34,.56),(s*.59,.34,.56)],.010,MATS['dark'])
    ...
    for s in (-1,1):
        ...
        ellipsoid('HEADREST_PAD_L' if s<0 else 'HEADREST_PAD_R',(s*.16,-.30,1.00),(.115,.055,.095),MATS['dark'],32,18)
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='BLOCKOUT_023_DETAIL_FIDELITY'; sc['D_tire_m']=.5
