...
OUT=os.path.join(ROOT,'bubble-bumper-v026.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    MATS['body_blue']=mat('Body Blue',(0.018,0.20,0.78),0.10,.24)
    MATS['accent_yellow']=mat('Custom Yellow',(1.0,0.68,0.015),0.03,.24)
    ...
    for s in (-1,1):
        create_wheel('WHEEL_FL' if s<0 else 'WHEEL_FR',s*.52,.82,.29,.235,.22,MATS['rubber'])
        create_wheel('WHEEL_RL' if s<0 else 'WHEEL_RR',s*.58,-.72,.33,.255,.25,MATS['rubber'])

    # Short front U bumper in the frontal plane: endpoints rise, center drops, as in the concept.
    bumper=[(-.72,1.40,.42),(-.82,1.40,.34),(-.68,1.40,.24),(0,1.40,.21),(.68,1.40,.24),(.82,1.40,.34),(.72,1.40,.42)]
    ...
    for s in (-1,1):
        torus('BUMPER_COLLAR_L' if s<0 else 'BUMPER_COLLAR_R',(s*.78,1.40,.37),.070,.014,MATS['accent_yellow'],rot=(0,math.pi/2,0))
    ...
    for s in (-1,1):
        ellipsoid('SIDE_POD_BLUE_BASE_L' if s<0 else 'SIDE_POD_BLUE_BASE_R',(s*.38,.00,.44),(.25,.58,.19),MATS['body_blue'],32,18)
        ellipsoid('SIDE_POD_YELLOW_PANEL_L' if s<0 else 'SIDE_POD_YELLOW_PANEL_R',(s*.58,.00,.48),(.105,.46,.135),MATS['accent_yellow'],32,18)
    ...
    # Authored C-shaped yellow fenders wrap the pod/wheel silhouette like the concept.
    for s in (-1,1):
        tube('POD_FENDER_C_L' if s<0 else 'POD_FENDER_C_R',[(s*.58,.55,.38),(s*.72,.35,.52),(s*.76,.02,.58),(s*.72,-.34,.52),(s*.58,-.55,.38)],.060,MATS['accent_yellow'])
        tube('POD_BLUE_SHOULDER_L' if s<0 else 'POD_BLUE_SHOULDER_R',[(s*.30,.50,.55),(s*.34,.10,.62),(s*.30,-.35,.55)],.050,MATS['body_blue'])
        tube('POD_PANEL_TOP_RIB_L' if s<0 else 'POD_PANEL_TOP_RIB_R',[(s*.58,.32,.61),(s*.58,-.28,.61)],.012,MATS['dark'])
        tube('POD_PANEL_BOTTOM_RIB_L' if s<0 else 'POD_PANEL_BOTTOM_RIB_R',[(s*.58,.32,.36),(s*.58,-.28,.36)],.014,MATS['metal_dark'])

    # Central blue cockpit shoulder shell creates a continuous cradle around the pilot.
    ellipsoid('COCKPIT_LEFT_SHOULDER',(-.20,.08,.58),(.16,.44,.12),MATS['body_blue'],40,24)
    ellipsoid('COCKPIT_RIGHT_SHOULDER',(.20,.08,.58),(.16,.44,.12),MATS['body_blue'],40,24)
    box('COCKPIT_FRONT_BRIDGE',(0,.48,.58),(.22,.10,.045),MATS['dark'],.025)
    box('COCKPIT_REAR_BRIDGE',(0,-.30,.62),(.22,.08,.050),MATS['body_blue'],.025)

    # Rounded fender lips tie the pods visually to the corner wheels.
    for s in (-1,1):
        tube('FRONT_FENDER_L' if s<0 else 'FRONT_FENDER_R',[(s*.56,.98,.40),(s*.68,.86,.48),(s*.68,.68,.40)],.035,MATS['body_blue'])
        tube('REAR_FENDER_L' if s<0 else 'REAR_FENDER_R',[(s*.62,-.52,.43),(s*.73,-.70,.52),(s*.67,-.88,.43)],.035,MATS['body_blue'])

    # Functional front struts visible through the bumper openings.
    for s in (-1,1):
        tube('FRONT_CHASSIS_STRUT_L' if s<0 else 'FRONT_CHASSIS_STRUT_R',[(s*.42,1.18,.27),(s*.30,.92,.38)],.018,MATS['metal_dark'])
        sphere('FRONT_STRUT_BOLT_L' if s<0 else 'FRONT_STRUT_BOLT_R',(s*.42,1.18,.27),.012,MATS['accent_yellow'])

    # Visible smooth hub rings and axle caps.
    for s in (-1,1):
        cylinder('FRONT_HUB_L' if s<0 else 'FRONT_HUB_R',(s*.525,.82,.29),.095,.035,MATS['metal'],rot=(0,math.pi/2,0))
        cylinder('FRONT_HUB_CAP_L' if s<0 else 'FRONT_HUB_CAP_R',(s*.548,.82,.29),.045,.042,MATS['dark'],rot=(0,math.pi/2,0))
        cylinder('REAR_HUB_L' if s<0 else 'REAR_HUB_R',(s*.585,-.72,.33),.105,.035,MATS['metal'],rot=(0,math.pi/2,0))
        cylinder('REAR_HUB_CAP_L' if s<0 else 'REAR_HUB_CAP_R',(s*.608,-.72,.33),.050,.042,MATS['dark'],rot=(0,math.pi/2,0))
    ...
    for s in (-1,1):
        ...
        box('HEADREST_PAD_L' if s<0 else 'HEADREST_PAD_R',(s*.16,-.30,1.00),(.115,.055,.085),MATS['rubber'],.035)

    ellipsoid('PILOT_HELMET',(0,.10,.98),(.16,.15,.16),MATS['pilot'],40,24)
    ellipsoid('PILOT_FACE',(0,.245,.99),(.115,.020,.075),MATS['accent_yellow'],28,18)
    for s in (-1,1):
        ellipsoid('PILOT_EYE_L' if s<0 else 'PILOT_EYE_R',(s*.052,.264,1.012),(.026,.008,.030),MATS['glass'],24,16)
        sphere('PILOT_PUPIL_L' if s<0 else 'PILOT_PUPIL_R',(s*.052,.273,1.012),.009,MATS['dark'])
    box('HELMET_YELLOW_STRIPE',(0,.10,1.135),(.028,.13,.018),MATS['accent_yellow'],.006)
    tube('HELMET_BLUE_BAND',[(-.14,.23,.92),(0,.25,.90),(.14,.23,.92)],.014,MATS['body_blue'])
    ...
    engine=loft_y('REAR_ENGINE',[(-.62,0,.56,.18,.14),(-.86,0,.59,.21,.16),(-1.05,0,.56,.17,.12)],MATS['metal'],20)
    ...
    for s in (-1,1):
        cylinder('REAR_ENGINE_CYL_L' if s<0 else 'REAR_ENGINE_CYL_R',(s*.24,-1.04,.62),.105,.30,MATS['metal'],rot=(math.pi/2,0,0))
        cylinder('REAR_ENGINE_CAP_L' if s<0 else 'REAR_ENGINE_CAP_R',(s*.24,-1.20,.62),.072,.018,MATS['dark'],rot=(math.pi/2,0,0))
        sphere('REAR_ENGINE_BOLT_L' if s<0 else 'REAR_ENGINE_BOLT_R',(s*.24,-1.215,.62),.012,MATS['accent_yellow'])
    ...
    for s in (-1,1):
        ...
        torus('EXHAUST_COLLAR_L' if s<0 else 'EXHAUST_COLLAR_R',(s*.39,-1.25,.74),.052,.012,MATS['accent_yellow'],rot=(math.pi/2,0,0))
    tube('REAR_HOUSING_LOWER_EDGE',[(-.18,-1.08,.30),(0,-1.10,.28),(.18,-1.08,.30)],.016,MATS['metal_dark'])

    tube('REAR_BAR_BLUE',[(-.63,-.96,.83),(0,-.98,.83),(.63,-.96,.83)],.045,MATS['body_blue'])
    for s in (-1,1):
        tube('REAR_BAR_YELLOW_L' if s<0 else 'REAR_BAR_YELLOW_R',[(s*.54,-.96,.83),(s*.68,-.96,.83)],.05,MATS['accent_yellow'])
        tube('EXHAUST_L' if s<0 else 'EXHAUST_R',[(s*.22,-.95,.62),(s*.32,-1.15,.68),(s*.39,-1.25,.74)],.045,MATS['metal'])
    ...
    box('GROUND',(0,0,-.035),(2.5,2.5,.02),MATS['dark'],.0)
    sc=bpy.context.scene
    sc['asset']='The Bubble Bumper'; sc['revision']='BLOCKOUT_026_AUTHORED_SHELL_DETAILS'; sc['D_tire_m']=.5
...
def lights_and_camera():
    ...
    cams={
      'top':((0,0,5.0),(0,.05,.50),3.60),
      'profile':((-4.0,.05,1.1),(0,.10,.60),2.70),
