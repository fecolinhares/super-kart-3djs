...
OUT=os.path.join(ROOT,'bubble-bumper-v027.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        ...
        ellipsoid('SIDE_POD_YELLOW_PANEL_L' if s<0 else 'SIDE_POD_YELLOW_PANEL_R',(s*.57,-.02,.37),(.12,.50,.105),MATS['accent_yellow'],32,18)
    ...
    for s in (-1,1):
        ...
        tube('POD_PANEL_TOP_RIB_L' if s<0 else 'POD_PANEL_TOP_RIB_R',[(s*.61,.34,.46),(s*.61,-.34,.46)],.010,MATS['dark'])
        tube('POD_PANEL_BOTTOM_RIB_L' if s<0 else 'POD_PANEL_BOTTOM_RIB_R',[(s*.61,.34,.29),(s*.61,-.34,.29)],.014,MATS['metal_dark'])
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='BLOCKOUT_027_CONCEPT_SIDE_SKIRTS'; sc['D_tire_m']=.5
