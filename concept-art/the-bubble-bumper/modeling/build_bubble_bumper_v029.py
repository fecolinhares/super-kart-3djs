...
OUT=os.path.join(ROOT,'bubble-bumper-v029.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        ...
        ellipsoid('SIDE_POD_YELLOW_PANEL_L' if s<0 else 'SIDE_POD_YELLOW_PANEL_R',(s*.58,-.02,.39),(.17,.58,.16),MATS['accent_yellow'],32,18)
    ...
    for s in (-1,1):
        ...
        ellipsoid('SIDE_POD_VISIBLE_BLUE_INSET_L' if s<0 else 'SIDE_POD_VISIBLE_BLUE_INSET_R',(s*.705,-.02,.39),(.030,.40,.082),MATS['body_blue'],40,24)
        tube('SIDE_POD_PANEL_BORDER_TOP_L' if s<0 else 'SIDE_POD_PANEL_BORDER_TOP_R',[(s*.725,.31,.48),(s*.725,-.31,.48)],.012,MATS['dark'])
        tube('SIDE_POD_PANEL_BORDER_BOTTOM_L' if s<0 else 'SIDE_POD_PANEL_BORDER_BOTTOM_R',[(s*.725,.31,.30),(s*.725,-.31,.30)],.012,MATS['dark'])
    ...
    ellipsoid('FRONT_COWL',(0,.70,.48),(.25,.30,.12),MATS['body_blue'],40,24)
    ...
    ellipsoid('PILOT_FACE',(0,.235,.89),(.115,.020,.075),MATS['accent_yellow'],28,18)
    for s in (-1,1):
        ellipsoid('PILOT_EYE_L' if s<0 else 'PILOT_EYE_R',(s*.052,.254,.912),(.026,.008,.030),MATS['glass'],24,16)
        sphere('PILOT_PUPIL_L' if s<0 else 'PILOT_PUPIL_R',(s*.052,.263,.912),.009,MATS['dark'])
    box('HELMET_YELLOW_STRIPE',(0,.08,1.035),(.028,.13,.018),MATS['accent_yellow'],.006)
    tube('HELMET_BLUE_BAND',[(-.14,.23,.82),(0,.25,.80),(.14,.23,.82)],.014,MATS['body_blue'])
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='BLOCKOUT_029_VISIBLE_POD_PANEL'; sc['D_tire_m']=.5
