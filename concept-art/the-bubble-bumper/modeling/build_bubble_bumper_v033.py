...
OUT=os.path.join(ROOT,'bubble-bumper-v033.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        box('SIDE_POD_BLUE_BASE_L' if s<0 else 'SIDE_POD_BLUE_BASE_R',(s*.38,.00,.44),(.24,.55,.18),MATS['body_blue'],.095)
        box('SIDE_POD_YELLOW_PANEL_L' if s<0 else 'SIDE_POD_YELLOW_PANEL_R',(s*.58,-.02,.39),(.125,.50,.15),MATS['accent_yellow'],.075)
    ...
    for s in (-1,1):
        ...
        box('SIDE_POD_VISIBLE_BLUE_INSET_L' if s<0 else 'SIDE_POD_VISIBLE_BLUE_INSET_R',(s*.725,-.02,.39),(.030,.29,.095),MATS['body_blue'],.028)
    ...
    box('FRONT_COWL',(0,.70,.48),(.25,.30,.12),MATS['body_blue'],.085)
    ...
    box('COCKPIT_LEFT_SHOULDER',(-.20,.08,.58),(.16,.44,.12),MATS['body_blue'],.075)
    box('COCKPIT_RIGHT_SHOULDER',(.20,.08,.58),(.16,.44,.12),MATS['body_blue'],.075)
    ...
    engine=box('REAR_ENGINE',(0,-.84,.58),(.28,.24,.20),MATS['metal'],.085)
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='BLOCKOUT_033_AUTHORED_PANEL_MESH'; sc['D_tire_m']=.5
