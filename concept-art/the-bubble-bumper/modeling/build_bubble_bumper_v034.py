...
OUT=os.path.join(ROOT,'bubble-bumper-v034.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    bumper=[(-.58,.93,.27),(-.72,1.02,.27),(-.84,1.20,.27),(-.82,1.40,.27),(-.65,1.52,.27),(0,1.56,.27),(.65,1.52,.27),(.82,1.40,.27),(.84,1.20,.27),(.72,1.02,.27),(.58,.93,.27)]
    ...
    for s in (-1,1):
        torus('BUMPER_COLLAR_L' if s<0 else 'BUMPER_COLLAR_R',(s*.80,1.39,.29),.070,.014,MATS['accent_yellow'],rot=(0,math.pi/2,0))
    ...
    engine=box('REAR_ENGINE',(0,-.78,.58),(.28,.20,.20),MATS['metal'],.085)
    ...
    for s in (-1,1):
        cylinder('REAR_ENGINE_CYL_L' if s<0 else 'REAR_ENGINE_CYL_R',(s*.24,-.98,.62),.105,.30,MATS['metal'],rot=(math.pi/2,0,0))
        cylinder('REAR_ENGINE_CAP_L' if s<0 else 'REAR_ENGINE_CAP_R',(s*.24,-1.14,.62),.072,.018,MATS['dark'],rot=(math.pi/2,0,0))
        sphere('REAR_ENGINE_BOLT_L' if s<0 else 'REAR_ENGINE_BOLT_R',(s*.24,-1.155,.62),.012,MATS['accent_yellow'])
    ...
    for s in (-1,1):
        ...
        torus('EXHAUST_COLLAR_L' if s<0 else 'EXHAUST_COLLAR_R',(s*.38,-1.18,.72),.052,.012,MATS['accent_yellow'],rot=(math.pi/2,0,0))

    tube('REAR_BAR_BLUE',[(-.58,-.88,.78),(0,-.90,.78),(.58,-.88,.78)],.045,MATS['body_blue'])
    for s in (-1,1):
        tube('REAR_BAR_YELLOW_L' if s<0 else 'REAR_BAR_YELLOW_R',[(s*.50,-.88,.78),(s*.62,-.88,.78)],.05,MATS['accent_yellow'])
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='BLOCKOUT_034_WHEEL_ANCHORS_AND_U'; sc['D_tire_m']=.5
