...
OUT=os.path.join(ROOT,'bubble-bumper-v038.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        cylinder('REAR_ENGINE_CYL_L' if s<0 else 'REAR_ENGINE_CYL_R',(s*.22,-.88,.62),.105,.30,MATS['metal_dark'],rot=(math.pi/2,0,0))
    ...
    for s in (-1,1):
        ...
        tube('EXHAUST_L' if s<0 else 'EXHAUST_R',[(s*.20,-.84,.62),(s*.29,-.98,.67),(s*.34,-1.07,.71)],.045,MATS['metal_dark'])
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='AAA_038_VISIBLE_DETAIL_AND_CLOSEUPS'; sc['D_tire_m']=.5
