...
OUT=os.path.join(ROOT,'bubble-bumper-v039.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    ellipsoid('PILOT_FACE',(0,.175,.89),(.115,.020,.075),MATS['accent_yellow'],28,18)
    for s in (-1,1):
        ellipsoid('PILOT_EYE_L' if s<0 else 'PILOT_EYE_R',(s*.052,.194,.912),(.026,.008,.030),MATS['glass'],24,16)
        sphere('PILOT_PUPIL_L' if s<0 else 'PILOT_PUPIL_R',(s*.052,.203,.912),.009,MATS['dark'])
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='AAA_039_PRIMARY_SHELL_AND_DRIVER'; sc['D_tire_m']=.5
