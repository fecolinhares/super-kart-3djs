...
OUT=os.path.join(ROOT,'bubble-bumper-v041.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    # Concept-faithful helmet face: yellow front panel, dark visor recess and two eyes.
    ellipsoid('PILOT_FACE',(0,.235,.89),(.115,.028,.075),MATS['accent_yellow'],32,20)
    box('PILOT_VISOR_RECESS',(0,.266,.895),(.085,.012,.035),MATS['dark'],.018)
    for s in (-1,1):
        ellipsoid('PILOT_EYE_L' if s<0 else 'PILOT_EYE_R',(s*.050,.282,.902),(.022,.009,.026),MATS['glass'],24,16)
        sphere('PILOT_PUPIL_L' if s<0 else 'PILOT_PUPIL_R',(s*.050,.292,.902),.008,MATS['dark'])
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='AAA_041_HELMET_FACE_IDENTITY'; sc['D_tire_m']=.5
