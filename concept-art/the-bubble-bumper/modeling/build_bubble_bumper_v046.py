...
OUT=os.path.join(ROOT,'bubble-bumper-v046.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        ...
        box('HEADREST_PAD_L' if s<0 else 'HEADREST_PAD_R',(s*.16,-.30,1.00),(.065,.035,.055),MATS['rubber'],.035)
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='AAA_046_COMPACT_REFERENCE_PROPORTIONS'; sc['D_tire_m']=.5
...
def lights_and_camera():
    ...
    cams={
      'top':((0,0,5.0),(0,.05,.50),3.85),
      'profile':((-4.0,.05,1.1),(0,.10,.60),2.85),
      'front':((0,4.0,1.0),(0,.55,.48),2.70),
      'rear':((0,-4.0,1.1),(0,-.70,.55),2.60),
