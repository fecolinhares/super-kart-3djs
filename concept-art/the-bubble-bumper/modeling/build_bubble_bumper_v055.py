...
OUT=os.path.join(ROOT,'bubble-bumper-v055.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        cylinder('REAR_BARREL_L' if s<0 else 'REAR_BARREL_R',(s*.22,-.82,.64),.105,.30,MATS['metal'],rot=(math.pi/2,0,0))
        cylinder('REAR_BARREL_CAP_L' if s<0 else 'REAR_BARREL_CAP_R',(s*.22,-.98,.64),.072,.018,MATS['dark'],rot=(math.pi/2,0,0))
