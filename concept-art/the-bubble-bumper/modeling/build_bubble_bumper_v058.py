...
OUT=os.path.join(ROOT,'bubble-bumper-v058.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    # Side orthographic driver identity and seated anatomy, visible from the negative-X camera.
    ellipsoid('SIDE_FACE_PANEL',(-.17,.06,.90),(.022,.11,.075),MATS['face'],32,20)
    box('SIDE_VISOR',(-.198,.08,.875),(.007,.065,.024),MATS['glass'],.008)
    ellipsoid('SIDE_EYE',(-.207,.11,.91),(.007,.022,.024),MATS['eye_white'],20,12)
    sphere('SIDE_PUPIL',(-.216,.11,.91),.007,MATS['dark'])
    ellipsoid('SIDE_CHEST',(-.13,0,.65),(.08,.16,.13),MATS['pilot'],28,18)
    tube('SIDE_UPPER_ARM',[(-.19,.05,.71),(-.25,.27,.66)],.030,MATS['pilot'])
    tube('SIDE_FOREARM',[(-.25,.27,.66),(-.13,.52,.66)],.028,MATS['pilot'])
    sphere('SIDE_GLOVE',(-.12,.54,.66),.038,MATS['accent_yellow'])
    tube('SIDE_THIGH',[(-.13,-.04,.52),(-.18,.18,.42),(-.10,.38,.32)],.040,MATS['pilot'])
    tube('SIDE_SHIN',[(-.10,.38,.32),(-.02,.58,.27)],.030,MATS['pilot'])
    box('SIDE_BOOT',(-.01,.63,.25),(.055,.10,.025),MATS['dark'],.012)
    ellipsoid('SIDE_SEAT_BACK',(0,-.20,.68),(.14,.08,.25),MATS['dark'],32,18)
    ...
    for s in (-1,1):
        cylinder('REAR_BARREL_L' if s<0 else 'REAR_BARREL_R',(s*.20,-.82,.68),.105,.30,MATS['metal'],rot=(math.pi/2,0,0))
        cylinder('REAR_BARREL_CAP_L' if s<0 else 'REAR_BARREL_CAP_R',(s*.20,-.98,.68),.075,.020,MATS['dark'],rot=(math.pi/2,0,0))
    ...
    sc=bpy.context.scene; sc['asset']='The Bubble Bumper'; sc['revision']='AAA_058_ORTHO_SIDE_DRIVER'; sc['source_concept']='concept-art/the-bubble-bumper/assets/the-bubble-bumper.jpg'; sc['source_orthos']='assets/reference-orthographic/{top,front,rear,side}.jpg'; sc['visual_gate']='pending'; sc['sol_gate']='pending'; sc['user_gate']='pending'
    return sc
