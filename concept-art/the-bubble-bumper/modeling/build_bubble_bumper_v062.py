...
OUT=os.path.join(ROOT,'bubble-bumper-v062.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    box('MANUAL_FRONT_GRILLE',(0,1.28,.31),(.23,.045,.08),MATS['metal_dark'],.025)
    for gx in (-.12,-.06,0,.06,.12): box('MANUAL_FRONT_SLOT_'+str(gx),(gx,1.23,.31),(.010,.012,.055),MATS['dark'],.004)
    for s in (-1,1):
        ellipsoid('MANUAL_FRONT_LIGHT_L' if s<0 else 'MANUAL_FRONT_LIGHT_R',(s*.22,1.12,.46),(.055,.020,.035),MATS['accent_yellow'],24,16)
    ...
    for s in (-1,1):
        tube('MANUAL_HEADREST_POST_L' if s<0 else 'MANUAL_HEADREST_POST_R',[(s*.15,-.27,.72),(s*.15,-.27,.93)],.015,MATS['metal_dark'])
        box('MANUAL_HEADREST_PAD_L' if s<0 else 'MANUAL_HEADREST_PAD_R',(s*.15,-.27,.96),(.075,.035,.055),MATS['dark'],.025)
    ...
    # Manual orthographic character identity in both FRONT and SIDE.
    ellipsoid('MANUAL_SIDE_FACE',(-.17,.06,.91),(.022,.11,.075),MATS['face'],32,20)
    box('MANUAL_SIDE_VISOR',(-.195,.08,.885),(.007,.065,.024),MATS['glass'],.008)
    ellipsoid('MANUAL_SIDE_EYE',(-.205,.11,.915),(.007,.022,.024),MATS['eye_white'],20,12)
    sphere('MANUAL_SIDE_PUPIL',(-.214,.11,.915),.007,MATS['dark'])
    box('MANUAL_HELMET_STRIPE',(0,.105,1.07),(.025,.12,.018),MATS['accent_yellow'],.006)
    ...
    for s in (-1,1):
        cylinder('MANUAL_EXHAUST_L' if s<0 else 'MANUAL_EXHAUST_R',(s*.20,-.82,.68),.12,.30,MATS['metal'],rot=(math.pi/2,0,0)); cylinder('MANUAL_EXHAUST_CAP_L' if s<0 else 'MANUAL_EXHAUST_CAP_R',(s*.20,-.98,.68),.060,.02,MATS['dark'],rot=(math.pi/2,0,0)); coil('MANUAL_SPRING_L' if s<0 else 'MANUAL_SPRING_R',(s*.50,-.55,.46),'z',.04,.10,.22,MATS['accent_yellow'])
    ...
    sc=bpy.context.scene; sc['asset']='The Bubble Bumper'; sc['revision']='AAA_062_ORTHO_HEADREST_WHEEL_REAR'; sc['source_orthos']='concept-art/the-bubble-bumper/assets/reference-orthographic/{top,front,rear,side}.jpg'; sc['visual_gate']='pending'; sc['sol_gate']='pending'; sc['user_gate']='pending'
    return sc
