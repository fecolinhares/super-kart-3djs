...
OUT=os.path.join(ROOT,'bubble-bumper-v065.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    for s in (-1,1):
        tube('ORTHO_POD_TOP_EDGE_L' if s<0 else 'ORTHO_POD_TOP_EDGE_R',[(s*.34,.28,.58),(s*.48,.02,.62),(s*.34,-.28,.58)],.030,MATS['accent_yellow'])
    ...
    for s in (-1,1):
        cylinder('EXHAUST_L' if s<0 else 'EXHAUST_R',(s*.20,-.82,.68),.155,.32,MATS['metal'],rot=(math.pi/2,0,0)); cylinder('EXHAUST_CAP_L' if s<0 else 'EXHAUST_CAP_R',(s*.20,-.98,.68),.095,.02,MATS['dark'],rot=(math.pi/2,0,0)); coil('SPRING_L' if s<0 else 'SPRING_R',(s*.50,-.55,.46),'z',.04,.10,.22,MATS['accent_yellow']); tube('REAR_ARM_L' if s<0 else 'REAR_ARM_R',[(s*.35,-.55,.43),(s*.62,-.60,.34)],.022,MATS['metal_dark'])
    ...
    # Reference front read: compact low crossbar and short returns, not a giant hoop.
    bumper=bpy.data.objects.get('MANUAL_BUMPER_U')
    if bumper: bumper.scale.x*=.72; bumper.scale.y*=.72
    tube('ORTHO_LOW_FRONT_BAR',[(-.50,1.27,.30),(0,1.29,.30),(.50,1.27,.30)],.055,MATS['body_blue'])
    for s in (-1,1): box('ORTHO_LOW_BAR_CAP_L' if s<0 else 'ORTHO_LOW_BAR_CAP_R',(s*.54,1.27,.30),(.045,.07,.045),MATS['accent_yellow'],.015)
    # Rear barrel scale matches the orthographic reference outlets.
    for name in ('MANUAL_EXHAUST_L','MANUAL_EXHAUST_R'):
        ob=bpy.data.objects.get(name)
        if ob: ob.scale*=.82

    box('GROUND',(0,0,-.035),(2.2,2.2,.02),MATS['concept_ground'],0)
    sc=bpy.context.scene; sc['asset']='The Bubble Bumper'; sc['revision']='AAA_065_ORTHO_U_BARRELS_PODS'; sc['reference_views']='assets/reference-orthographic/{top,front,rear,side}.jpg'; sc['visual_gate']='pending'; sc['sol_gate']='pending'; sc['user_gate']='pending'
    return sc
