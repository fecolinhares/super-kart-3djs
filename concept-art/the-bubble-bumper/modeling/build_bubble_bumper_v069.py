...
OUT=os.path.join(ROOT,'bubble-bumper-v069.blend')
COL=None
MATS={}
...
def create_scene():
    ...
    # Side-facing concept helmet panel for true lateral-view identity.
    ellipsoid('PILOT_SIDE_FACE_L',(-.155,.08,.89),(.018,.105,.070),MATS['face'],28,18)
    box('PILOT_SIDE_VISOR_L',(-.178,.10,.88),(.008,.075,.035),MATS['dark'],.010)
    ellipsoid('PILOT_SIDE_EYE_L',(-.187,.12,.905),(.008,.020,.022),MATS['eye_white'],20,12)
    sphere('PILOT_SIDE_PUPIL_L',(-.196,.12,.905),.007,MATS['dark'])
    ...
    ellipsoid('DRIVER_NEAR_CHEST',(-.14,.00,.64),(.070,.16,.13),MATS['pilot'],28,18)
    ellipsoid('DRIVER_NEAR_HIP',(-.14,-.04,.50),(.080,.16,.075),MATS['pilot'],28,18)
    # Near-side profile driver anatomy: arm, elbow, knee and boot must read in lateral view.
    tube('DRIVER_NEAR_UPPER_ARM',[(-.17,.06,.72),(-.25,.28,.67)],.035,MATS['pilot'])
    tube('DRIVER_NEAR_FOREARM',[(-.25,.28,.67),(-.12,.52,.66)],.032,MATS['pilot'])
    sphere('DRIVER_NEAR_GLOVE',(-.105,.55,.66),.040,MATS['pilot_yellow'])
    tube('DRIVER_NEAR_THIGH',[(-.10,-.02,.50),(-.18,.20,.42),(-.10,.38,.34)],.045,MATS['pilot'])
    tube('DRIVER_NEAR_SHIN',[(-.10,.38,.34),(-.02,.57,.28)],.035,MATS['pilot'])
    box('DRIVER_NEAR_BOOT',(-.01,.62,.26),(.055,.10,.025),MATS['dark'],.015)
    tube('DRIVER_NEAR_YELLOW_SLEEVE',[(-.19,.09,.73),(-.25,.27,.68)],.012,MATS['accent_yellow'])
    ...
    sc['asset']='The Bubble Bumper'; sc['revision']='AAA_045_BIDIRECTIONAL_DRIVER_FACE'; sc['D_tire_m']=.5
...
def main():
    ...
    render_dir=os.path.join(ROOT,'renders-v069'); os.makedirs(render_dir,exist_ok=True)
    bpy.ops.wm.save_as_mainfile(filepath=OUT,compress=True)
    for name in ('top','profile','front','rear','isometric'):
        cam=bpy.data.objects.get('CAM_'+name.upper())
        if not cam: raise RuntimeError('missing camera '+name)
        sc.camera=cam
        sc.render.filepath=os.path.join(render_dir,name+'.png')
        bpy.ops.render.render(write_still=True)
    bpy.ops.wm.save_as_mainfile(filepath=OUT,compress=True)
    print('BUBBLE_BUMPER_V069_OK',OUT,'OBJECTS',len(bpy.data.objects),'RENDERS',len([n for n in os.listdir(render_dir) if n.endswith('.png')]))
