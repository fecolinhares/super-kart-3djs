// Diag final: raycast na banda oliva com câmera chase real + THREE importado
...
(async () => {
  ...
  const report = await page.evaluate(async () => {
    ...
    const out = [];
    // NDC y ~0.35-0.45, x variados — onde a banda aparece na chase cam
    for (const [nx, ny] of [[-0.3, 0.4], [0.3, 0.4], [0.45, 0.42], [-0.45, 0.42], [0.0, 0.38]]) {
      ...
      out.push(hits.slice(0, 2).map(h => {
        ...
        return { d: Math.round(h.distance), c: chain.slice(0,2).join('<'), col: m && m.color ? '#' + m.color.getHexString() : null, map: !!(m && m.map), fog: m && m.fog, tm: m && m.toneMapped, p: [Math.round(h.point.x), Math.round(h.point.y), Math.round(h.point.z)] };
      }));
    }
    return out;
  });
