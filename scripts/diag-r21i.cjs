// Diag R21i: a colina verde está a ~100-200m — raycast MAIS LONGE, ignorando hits < 60m
...
(async () => {
  ...
  const report = await page.evaluate(async () => {
    ...
    ray.setFromCamera(new THREE.Vector2(0.17, -0.17), cam);
    const hits = [];
    scene.traverse((o) => {
      if (!o.isMesh) return;
    ...
    // só hits >= 40m (a colina está longe)
    return hits.filter(h => h.d >= 40).slice(0, 6);
  });
