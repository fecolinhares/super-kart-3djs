// Diag R21j: raycast varrendo NDCs procurando hits 60-300m (a colina)
...
(async () => {
  ...
  const report = await page.evaluate(async () => {
    ...
    const found = [];
    for (let nx = -0.8; nx <= 0.8; nx += 0.15) {
      for (let ny = -0.45; ny <= -0.05; ny += 0.1) {
        ...
        scene.traverse((o) => {
          ...
          for (const h of r) {
            if (h.distance < 60 || h.distance > 320) continue;
        ...
        if (best) found.push({ ndc: [nx.toFixed(2), ny.toFixed(2)], ...best });
      }
    }
    return found;
  });
