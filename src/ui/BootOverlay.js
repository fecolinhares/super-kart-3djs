...
export function createBootOverlay() {
  ...
  if (!root) {
    ...
    style.textContent = `
      #sk3d-boot{position:fixed;inset:0;z-index:1000;display:grid;place-items:center;background:radial-gradient(120% 90% at 50% 8%,#243b63 0%,#10182d 46%,#050812 100%);color:#f7fbff;transition:opacity .45s ease;font-family:system-ui,-apple-system,sans-serif;pointer-events:auto}
      ...
      .sk3d-boot-logo{font-weight:950;letter-spacing:.16em;font-size:clamp(30px,8vw,72px);line-height:.9;text-align:center;background:linear-gradient(#fff,#ffe18a 55%,#f19b39);-webkit-background-clip:text;background-clip:text;color:transparent;filter:drop-shadow(0 4px 12px #0009)}
      .sk3d-boot-logo span{display:block;font-size:.48em;letter-spacing:.48em;margin-left:.48em}
      .sk3d-boot-bar{height:5px;width:100%;border-radius:99px;overflow:hidden;background:#ffffff20}
      .sk3d-boot-bar i{display:block;width:0;height:100%;border-radius:inherit;background:linear-gradient(90deg,#2ec4ff,#ffd166);box-shadow:0 0 14px #ffd166aa;transition:width .25s ease}
      .sk3d-boot-step{min-height:1.2em;color:#d8e7f5aa;font-size:12px;letter-spacing:.22em;text-transform:uppercase;text-align:center}
